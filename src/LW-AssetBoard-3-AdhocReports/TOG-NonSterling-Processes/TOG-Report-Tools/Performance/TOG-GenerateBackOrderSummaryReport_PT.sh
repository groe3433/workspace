...
NotEncryptedOutputFileName="Performance_NoEncryption_BackOrderSummary_`date +%Y%m%d%H%M%S`.csv";
OutputFileName="Performance_BackOrderSummary_`date +%Y%m%d%H%M%S`.csv";
Script2="TOG-GenerateBackOrderSummaryReport_PT.sql";
...
echo "Attached is the BackOrderSummary PT Test File..." | mailx -s "Performance BackOrderSummary Report" -a $OutputFileName $REPORT_EMAIL;
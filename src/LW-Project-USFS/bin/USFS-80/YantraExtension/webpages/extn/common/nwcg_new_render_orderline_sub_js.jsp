<!-- /extn/common/nwcg_new_render_orderline_sub_js.jsp -->
...
/*
	xmlDoc e.g.:
	<RequestNumbers><RequestNoLine RequestNo="S-000011.1/><RequestLine RequestNo="S-000011.2/> ...
*/
function populateMultipleRows (elm, xmlDoc) {
	...
	for (i=0;  i < nl.length; i++) {
		//alert("requestNumbers from XML: "+nl[i].getAttribute("RequestNo"));
...
// TODO: extn.js has this as checkStringForNull(str) already
...
function populateBlankRowForSubstitutionUI(documentType,driverDate,strPCData,strUOMData,counter, requestNo)
{
	...
	if(!bOtherIssues)
	{
		//No validation done on request number column for substitution
		requestNoCol = '<td><INPUT class=unprotectedoverrideinput name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnRequestNo" dataType="STRING" maxLength=12 size=12  value="'; 
		requestNoCol = requestNoCol + requestNo + '"/>';
	}
	else
	{
		requestNoCol = '<td><INPUT class=unprotectedoverrideinput  name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnRequestNo" maxLength=12 size=12 dataType="STRING" value="';
		requestNoCol = requestNoCol + requestNo + '"/>';
	}
	...
	var strQtyRfi = '<td class="tablecolumn" style="width:120px">' +
							'<A onclick="checkAvailableRFI(this)" href=""><INPUT type="text" class=protectedinput tabindex="-1" title="Currently Available RFI Quantity\rClick for details." name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnQtyRfi" dataType="STRING"/></A>';	
	...
	if (driverDate == "02") 
	{ 
		strDate = strDate + '<input type="text" tabindex="7" OldValue="" style=\'width:60px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine_'+counter+'/@ReqDeliveryDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/yantra/console/icons/calendar.gif" />';
	} 
	else 
	{ 
		strDate = strDate + '<input type="text" tabindex="7" OldValue="" style=\'width:60px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine_'+counter+'/@ReqShipDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/yantra/console/icons/calendar.gif"/>';
	} 
	...
	for (i = 0 ; i < InputList.length ; i++)
	{
		...
		else if (InputList[i].name.indexOf('/Extn/@ExtnBackorderedQty') != -1) 
		{
			if (InputList[i].value == null || InputList[i].value == 'undefined' || InputList[i].value == '') 
			{
				...
				//Backorder qty has a value, if its on a radio kit item
				//pop up an alert saying that BO is not alllowed for
				//radio kit items
				if (sysNoRequired) 
				{
		...
		else if ( (InputList[i].name.indexOf('/Extn/@ExtnRequestNo') != -1) ||
				  (InputList[i].name.indexOf('/Item/@ItemID') != -1) ||
				  (InputList[i].name.indexOf('/OrderLineTranQuantity/@TransactionalUOM') != -1) ||
				  (InputList[i].name.indexOf('/OrderLineTranQuantity/@OrderedQty') != -1) ||
				  (InputList[i].name.indexOf('/@ReqShipDate') != -1) ||
				  (InputList[i].name.indexOf('@ExtnSystemNo') != -1))
		{		
			if (InputList[i].name.indexOf('@ExtnSystemNo') != -1) 
			{
				// The following RegEx matches for non-zero length alphanumeric 
				// values
				var regExp = /^[0-9a-zA-Z]+$/i;
				var clientSideSysNo = InputList[i].value;
				
				// System Number entry REQUIRED! Validation done to
				// ensure that the entry is alpha numeric.
				if (sysNoRequired == 1) {
				...
				{
					// System Number entry not required, validation done though
					// if one is entered for a non 4390 item.
...
<!-- End /extn/common/nwcg_new_render_orderline_sub_js.jsp -->

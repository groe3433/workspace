...
<tr>
<td class=tablecolumn noWrap><input readonly=true class=protectedtext size=10 name="xml:/Item/@Node" Value="<%=curUsr.getAttribute("Node")%>"></td>
</tr>

<tr>
<td class="searchlabel" ><yfc:i18n>ReservationID</yfc:i18n></td>
</tr>
<tr>
    <td nowrap="true" class="searchcriteriacell" >
        <select name="xml:/Item/@ReservationIDQryType" class="combobox" >
            <yfc:loopOptions binding="xml:/QueryTypeList/StringQueryTypes/@QueryType" 
                name="QueryTypeDesc" value="QueryType" selected="xml:/Item/@ReservationIDQryType"/>
        </select>
        <input type="text" class="unprotectedinput" <%=getTextOptions("xml:/Item/@ReservationID") %> />
		<img class="lookupicon" onclick="callLookup(this,'NWCGReservationLookup')" <%=getImageOptions(YFSUIBackendConsts.LOOKUP_ICON, "Search_for_Reservation_ID") %> />
    </td>
</tr>
...
<tr>
    <td nowrap="true" class="searchcriteriacell" >
		...
		<IMG class=lookupicon onclick="templateRowCallItemLookup(this,'ItemID','ProductClass','UnitOfMeasure','item')" alt="Search for Item" src="<%=request.getContextPath()%>/console/icons/lookup.gif">
    </td>
</tr>

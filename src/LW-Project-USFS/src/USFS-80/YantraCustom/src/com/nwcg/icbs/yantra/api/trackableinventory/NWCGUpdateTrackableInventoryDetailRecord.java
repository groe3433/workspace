...
public class NWCGUpdateTrackableInventoryDetailRecord implements YIFCustomApi{
	...
	public Document updateTrackableInventoryDetailRecord(YFSEnvironment env, Document inDoc)
	throws Exception {
		
//		System.out.println("NWCGUpdateTrackableInventoryDetailRecord: updateTrackableInventoryDetailRecord: inDoc " + XMLUtil.getXMLString(inDoc));		
	...
	/**
	 * This function obtains Item details pertaining to the trackable Item key passed.
	 * @param env
	 * @param trackableItemKey
	 * @return
	 * @throws Exception
	 */
	private Document getTrackableItemRecord(YFSEnvironment env, String trackableItemKey)
	throws Exception
	{
		...
		Document trackOutDoc = CommonUtilities.invokeService(env, NWCGConstants.NWCG_GET_TRACKABLE_ITEM_LIST_SERVICE, tirDocument);
//		System.out.println("NWCGUpdateTrackableInventoryDetailRecord: getTrackableInventoryRecord: trackOutDoc : " + XMLUtil.getXMLString(trackOutDoc));		
		return trackOutDoc;
	}
	...
	throws Exception
	{
		...
		Document invOutDoc = CommonUtilities.invokeAPI(env, NWCGConstants.API_GET_SERIAL_LIST, invInDocument);
//		System.out.println("NWCGUpdateTrackableInventoryDetailRecord: getInventoryDetails: invOutDoc : " + XMLUtil.getXMLString(invOutDoc));		
		return invOutDoc;
	}
	
	/**
	 * This function updates the Trackable Item Details for the trackable item. It updates the NWCG_TRACKABLE_ITEM table.
	 * @param env
	 * @param tirInDoc
	 * @return
	 * @throws Exception
	 */
	private Document updateTrackableItemDetails(YFSEnvironment env, Document tirInDoc)
	throws Exception
	{
		Document tirOutDoc = CommonUtilities.invokeService(env, "NWCGChangeTrackableInventoryRecordService", tirInDoc);
//		System.out.println("NWCGUpdateTrackableInventoryDetailRecord: updateTrackableItemDetails: tirOutDoc : " + XMLUtil.getXMLString(tirOutDoc));		
		return tirOutDoc;
	}
	
	
	/**
	 * This function updates the Tag Details for the trackable item. It updates the YFS_INVENTORY_TAG table.
	 * @param env
	 * @param tagInDoc
	 * @return
	 * @throws Exception
	...
	throws Exception
	{
		...
		String strTrackableItemKey = rootElement.getAttribute("TrackableItemKey");
		...
		String strManufacturingDate = rootElement.getAttribute("ManufacturingDate");
		
		String strItemId = null;
		String strNode = null;
		String strSerialNo = null;
		Document tirDoc = getTrackableItemRecord(env, strTrackableItemKey);
		Element trackOutElem = (Element) tirDoc.getDocumentElement().getElementsByTagName("NWCGTrackableItem").item(0);
		...
		Element serialElem = (Element) invDetailsDoc.getDocumentElement().getElementsByTagName("Serial").item(0);
		if(serialElem != null)
		{
		...
		Element rootChangeLocInvAttr = changeLocInvAttrDoc.createElement("ChangeLocationInventoryAttributes");
		changeLocInvAttrDoc.appendChild(rootChangeLocInvAttr);
		
		rootChangeLocInvAttr.setAttribute("EnterpriseCode", NWCGConstants.ENTERPRISE_CODE);
		...
		Element invItemFrom = changeLocInvAttrDoc.createElement("InventoryItem");		
		...
		Element serialListFrom = changeLocInvAttrDoc.createElement("SerialList");		
		Element serialDetailFrom = changeLocInvAttrDoc.createElement("SerialDetail");
		...
		Document tagOutDoc = CommonUtilities.invokeAPI(env, NWCGConstants.API_CHG_LOCN_INV_ATTR, changeLocInvAttrDoc);		
//		System.out.println("NWCGUpdateTrackableInventoryDetailRecord: updateTagDetails: tagOutDoc: " + XMLUtil.getXMLString(tagOutDoc));		
		return tagInDoc;
	}

...
public class NWCGTransferLineFullBackOrder implements YCPDynamicConditionEx 
{
	private Map map = null;

	public boolean evaluateCondition(YFSEnvironment env, String name, Map mapData, Document doc) 
	{
		//System.out.println("NWCGTransferLineFullBackOrder:evaluateCondition:Input XML\n" + XMLUtil.getXMLString(doc));
		...
		try 
		{
			if (orderLineKey != null && !orderLineKey.equals(NWCGConstants.EMPTY_STRING)) 
			{
				...
				result = (Float.parseFloat(orderedQty) == 0.0 && Float.parseFloat(extnBackorderedQty) > 0)? true : false;
			}
		} 
		catch (Exception ep){
			result = false;

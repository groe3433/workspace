...
public class EvaluateCondition implements YCPDynamicConditionEx {
	...
	public boolean evaluateCondition(YFSEnvironment env, String name,
		...
		try {
			...
			// By default it is set to true, need to pass explictly a false value  
			strXPathValueMapping = (String) map.get(XPATH_VALUE_MAPPING);
			//logger.verbose("strXPathValueMapping ==>"+strXPathValueMapping);

			bXPathValueMapping = ((strXPathValueMapping != null && strXPathValueMapping.equals("false")) ? false : true);
			...
			if (entityName.equalsIgnoreCase(KEY_ORDER_LINE)) {
				...
				if (template != null) {
					...
					env.setApiTemplate(API_ORDERLINE, XMLUtil
				...
				if (template != null) {
					...
					env.setApiTemplate(API_SHIPMENT, XMLUtil.getDocument(template));
				}
				...
				if (template != null) {
					...
					env.setApiTemplate(API_ORDER, XMLUtil
							.getDocument(template));
				}
				// Function to get Shipment Details
			...
			for (int lstIndex = 0; lstIndex < lstSize; lstIndex++) {
				...
				if(!bXPathValueMapping)
				{// if its not XPath value mapping - checking for the nodes if no node returned then the configured value
				// doesnt exists 
				...
				else
				{
					...
					xPathKeyActualValue = XPathUtil.getString(outputXMLDoc, xPath);
					//logger.verbose("  xPath == > " +xPath);
					//logger.verbose("  xPathKeyActualValue == > " +xPathKeyActualValue );
//					printDOMTree(outputXMLDoc.getDocumentElement());

					xPathKeyExpectedValue = (String) map.get(lstXPaths.get(lstIndex));
				}
				//logger.verbose("  xPathKeyActualValue == > " +xPathKeyActualValue);
				//logger.verbose("  xPathKeyExpectedValue == > " +xPathKeyExpectedValue);
				if (operator != null && operator.length() > 0
					...
					if (operator.equalsIgnoreCase(AND_OPERATOR)) {
						...
						if (xPathKeyActualValue.equalsIgnoreCase(xPathKeyExpectedValue)) {
							result = true;
						} else {
							result = false;
							break;
	...
	private void checkValues(int lstSize, String entityName, String operator) {
		...
		if (entityName == null || entityName.length() == 0) {
			...
			throw new RuntimeException(ResourceUtil.resolveMsgCode("USFS_COMMON_CONDITION_XPATH2"));

		} else if (lstSize == 0) {
			
			throw new RuntimeException(ResourceUtil.resolveMsgCode("USFS_COMMON_CONDITION_XPATH1"));
			
		} else if (lstSize > 1 && (operator == null || operator.length() == 0)) {

			throw new RuntimeException(ResourceUtil.resolveMsgCode("USFS_COMMON_CONDITION_XPATH3"));
	...
	private Document getOrderDetail(YFSEnvironment env, Map mapData,
		...
		if (orderHeaderKey == null || orderHeaderKey.length() == 0) {
			...
			throw new RuntimeException(ResourceUtil.resolveMsgCode("USFS_COMMON_CONDITION_XPATH4"));

		} else {

			// Input XML for fetching order details
			inputXMLString = "<Order OrderHeaderKey='"+orderHeaderKey+ "' EnterpriseCode='"+enterpriseCode+"' />";

			// Parsing the inputXMLString to get document object
			inputXMLDoc = XMLUtil.getDocument(inputXMLString);

			// Calling getOrderDetails
			outputXMLDoc = CommonUtilities.invokeAPI(env, API_ORDER, inputXMLDoc);
	...
	private Document getShipmentDetail(YFSEnvironment env, Map mapData,
		...
		if (shipmentKey == null || shipmentKey.length() == 0) {
			...
			throw new RuntimeException(ResourceUtil.resolveMsgCode("USFS_COMMON_CONDITION_XPATH5"));
			...
			outputXMLDoc = CommonUtilities.invokeAPI(env, API_SHIPMENT,inputXMLDoc);
	...
	private List isKeyXPath(boolean bXPathValueMapping) {
		...
		while (eit.hasNext()) {
			...
			//logger.verbose("isKeyXPath  indexOf==>"+ (propKey.indexOf(CONFIGURED_XPATH_NAME) != -1));

			if( // if it is Xpath - Value mapping, we need to push all the variables
			// with / in it to a List
				( bXPathValueMapping && propKey.charAt(0) == '/')  || 
					// else look for configured xpath name
				( (!bXPathValueMapping) && (propKey.indexOf(CONFIGURED_XPATH_NAME) != -1) ) 
			   )
			{

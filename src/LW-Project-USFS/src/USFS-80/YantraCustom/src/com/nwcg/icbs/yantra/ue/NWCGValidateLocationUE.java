...
public class NWCGValidateLocationUE implements WMSBeforeCreateCountRequestUE {
	...
	private static Logger log = Logger.getLogger(NWCGValidateLocationUE.class.getName());
...
/* <CountRequest DocumentType="3001" EnterpriseCode="" FinishNoLaterThan=""
    ...
    FromLocation="STOR1-01010201" IgnoreOrdering="Y"
    ItemClassification1="" ItemClassification2="" ItemClassification3=""
    ItemID="" LevelNumber="" LocationId="" ModifyFlag="" Node="CORMK"
    PalletId="" PipelineKey="" Priority="3" ProductClass=""
    ReasonText="" ReceiptHeaderKey="" ReceiptNo=""
    RequestType="PHYSICAL-COUNT" RequestTypeDescription=""
...
*/		    
	...
	public Document beforeCreateCountRequest(YFSEnvironment env, Document inXML) throws YFSUserExitException
	{
		if(log.isVerboseEnabled()) log.verbose("Input XML "+ XMLUtil.getXMLString(inXML));
		...
		System.out.println(" WMSBeforeCreateCountRequestUE Input XML " + XMLUtil.getXMLString(inXML));
    	...
    	if (RequestType.equals("PHYSICAL-COUNT")) 
    	{
    	  if (OriginalTaskId.length() > 0)
       	  {	 
    		Document taskXML = XMLUtil.createDocument("DeleteCountResultRecordsForTask");
			taskXML.getDocumentElement().setAttribute("Action","Modify");
			taskXML.getDocumentElement().setAttribute("TaskId",OriginalTaskId);
			taskXML.getDocumentElement().setAttribute("VarianceAccepted","I");
			
			Document taskoutputXML = CommonUtilities.invokeAPI(env,"deleteCountResultRecordsForTask",taskXML);
       	  }
    	  ...
    	  {	  
		   ...
		   outDoc = CommonUtilities.invokeAPI(env,"getLocationDetails",inDoc);
		   System.out.println(" Location Details XML " + XMLUtil.getXMLString(outDoc));	
    	  ...
    	  if (FreezeIn.equals("Y") || FreezeOut.equals("Y"))
    	  {
    		...
    		if (OriginalCountReqNo.length() > 0 && CountReqNo.length() > 0)
    		{
    			...
    			{
    				//Format for subsequent count requests (xxxxx.1, xxxxx.2 etc.)
    				String numericPortion = OriginalCountReqNo.substring(indexOfPeriod + 1);
    				String countRequestNumberBeforePeriod = OriginalCountReqNo.substring(0, indexOfPeriod);

    				try{
    					int integerSuffix = Integer.parseInt(numericPortion);
    					NewCountReqNo = countRequestNumberBeforePeriod + "." + (integerSuffix + 1);  
    				}catch(NumberFormatException nfe){
    					throw new NWCGException("NWCGValidateLocationUE: Could not parse Count Request Number.");
    				}
    			}
			...
			inputXML.getDocumentElement().setAttribute("SequenceType","COUNTREQUEST");
			inputXML.getDocumentElement().setAttribute("CacheID",Node);
			System.out.println("Input XML that was just created: "+ XMLUtil.getXMLString(inputXML));
			
			Document outputXML = CommonUtilities.invokeService(env,"NWCGGenerateReceiptNoService",inputXML);
			...
			doc.getDocumentElement().setAttribute("CodeType","NWCG_PC_YEAR");
			Document CDoc = CommonUtilities.invokeAPI(env,"getCommonCodeList",doc);
			
			String nwcg_pc_year = XPathUtil.getString(CDoc,"/CommonCodeList/CommonCode/@CodeValue");
			System.out.println("Common Code Return Value "+ nwcg_pc_year);
			...
			if (OriginalCountReqNo.length() > 0) {
				...
				// QC1156 - Limiting the CountRequest Length to 40 character
				if (NewCountReqNo.length() > 40) {
					System.out.println(" @@@@ CountReqNo greater than allowed limit");
					...
					System.out.println(" @@@@ The NewCountReqNo within limited 40 character count : "+ NewCountReqNo);
				}
			}
			else
			{
		...
		System.out.println(" WMSBeforeCreateCountRequestUE Output XML " + XMLUtil.getXMLString(inXML));
		return inXML;
	}
}

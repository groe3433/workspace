...
public class NWCGRFPrepareInputProcessReturnNRFI implements YIFCustomApi {
	...
	public Document rfPrepareInputProcessReturnNRFI(YFSEnvironment env, Document inXML)
	throws Exception {
...
private void updateNWCGIncidentReturn(YFSEnvironment env,Document processReturnOutDoc)throws Exception {
		...
		for(int i=0;i<lsReceiptLineList.getLength();i++)
		{
			...
			if(eleReceiptLine.hasAttribute("LPNNo"))
			{
				...
				el_ReturnLine.setAttribute("QuantityNRFI",eleReceiptLine.getAttribute("QtyReturned"));
				...
				if(lsSerialInfo.getLength()>0)
				{
					...
					eleUpdate.setAttribute("SerialStatus",NWCGConstants.SERIAL_STATUS_NRFI);
					eleUpdate.setAttribute("SerialStatusDesc", NWCGConstants.SERIAL_STATUS_NRFI_DESC);
		...
		{	
			...
			while(itr.hasNext())
			{
				...
				eleupdateCompDoc.setAttribute("SerialStatus",NWCGConstants.SERIAL_STATUS_NRFI);
				eleupdateCompDoc.setAttribute("SerialStatusDesc", NWCGConstants.SERIAL_STATUS_NRFI_DESC);

...
public class NWCGSerialRecordReceiptCacheTransfer extends NWCGSerialRecordReceipt implements NWCGSerialRecord 
{
	public Document insertSerialRecord(YFSEnvironment env, Document inDoc) throws Exception 
	{
		if(this.preCheckInput(inDoc))
		{
			...
			for(int j = 0; j < receiptLineList.getLength(); j++)
			{
				...
				//RECEIVE_RECEIPT.0006 has OrderType in it
				String orderType = ((Element)receiptLineElement.getElementsByTagName("Order").item(0)).getAttribute("OrderType");
				receiptMap.put("OrderType", orderType); 

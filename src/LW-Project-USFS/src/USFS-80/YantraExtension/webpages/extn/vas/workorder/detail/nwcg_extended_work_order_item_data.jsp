...
<%  
	...
	//System.out.println("TOP strSIGC=["+strSIGC+"]");
%>
	...
	<tr>
		...
		<%} else {%>
			<td class="tablecolumn" nowrap="true" >
				...
				<input type="text" class="unprotectedinput" <%=getTextOptions("xml:/WorkOrder/@ItemID")%> onblur="fetchDataWithParams(this,'getItemList',populateIncidentItemDetailsForWO,setItemParam(this))" />
		...
		{%>
			<td nowrap="true" class="tablecolumn">
				<select <%=getComboOptions("xml:/WorkOrder/@Uom")%> class="combobox" onblur="javascript:if(this.value!='') {fetchDataWithParams(this,'getItemDetailsForWO',populateItemDetailsForWO,setItemParamForWO(this))}">
		...
		<%}else{%>
			<td nowrap="true" class="tablecolumn">
			<select <%=getComboOptions("xml:/WorkOrder/@ProductClass")%> class="combobox" onblur="javascript:if(this.value!='') {fetchDataWithParams(this,'getQtyAvailToBuild',populateQtyAvailToBuild,setItemParamQtyAvailToBuild(this))}">
	...
	<tr>
		...
		<%}else{%>
			...
			<% if(strSIGC.equals("KIT")) {%>
				<!-- display this ONLY for Kitting -->
				<td class="detaillabel" ><yfc:i18n>Qty_Avail_To_Build</yfc:i18n></td>
				<td nowrap="true" >
					<input type="text" class="protectedinput" readonly="true" <%=getTextOptions("xml:/WorkOrder/@QtyAvailToBuild")%>/>
				</td>
				<Input type="hidden" name="xml:/WorkOrder/@QtyAvailToBuild" value="<%=resolveValue("xml:/WorkOrder/@QtyAvailToBuild")%>"/>
			<%}%>
		    <!-- Bottom of CR 58 JSK -->

			<td class="detaillabel" ><yfc:i18n>Requested_Qty</yfc:i18n></td>
			<td nowrap="true" >
				<input type="text" class="unprotectedinput" onBlur="javascript:if(parseFloat(document.getElementById('xml:/WorkOrder/@QtyAvailToBuild').value) < parseFloat(this.value)){alert('WARNING: Requested Qty is greater than Qty Available to Build the Kit.\n If the Requested Qty is correct, please click Proceed.');}" <%=getTextOptions("xml:/WorkOrder/@QuantityRequested")%>/>
			</td>

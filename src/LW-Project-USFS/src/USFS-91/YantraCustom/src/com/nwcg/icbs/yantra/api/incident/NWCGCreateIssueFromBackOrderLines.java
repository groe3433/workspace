...
public class NWCGCreateIssueFromBackOrderLines implements YIFCustomApi {
	/**
	 * Sample Input XML <OrderLineList IgnoreOrdering="Y"> <OrderLine
	 * IncidentNo="WY-SHF-000500" IncidentYear="2010" NewIssueQty="2"
	 * OrderLineKey="201005211137354312946" YFC_NODE_NUMBER="1"/> <OrderLine
	 * IncidentNo="WY-SHF-000500" IncidentYear="2010" NewIssueQty="3"
	 * OrderLineKey="201005221640094314606" YFC_NODE_NUMBER="3"/>
	 * </OrderLineList>
	 * 
	 * @param env
	 * @param inDoc
	 *            The OrderLineList input org.w3c.dom.Document with the
	 *            backorder qty for which the issue needs to be created
	 * @return Document The createOrder output after the issue has been created
	 * @throws Exception
	 */
	
	private static YFCLogCategory logger = NWCGApplicationLogger.instance(NWCGCreateIssueFromBackOrderLines.class);
	
	public Document createIssuesFromBackOrderLines(YFSEnvironment env,
		...
		/*
		 * Validate the inDoc's Order/OrderLines/OrderLine elements before
		 * continuing with creationg of the back order, in case we find that the
		 * input is not for any reason. Then the API will throw an exception.
		 */
		validateLines(env, inDoc);

		/*
		 * Loop over the Order/OrderLines/OrderLine Elements in the NodeList
		 * from the inDoc containing a Node for all the order line(s) in the
		 * input that need to be added to a BO order.
		 */
		...
		for (int i = 0; i < inputDocOrderLineList.getLength(); i++) {
			...
			if (logger.isVerboseEnabled()) {
				logger.verbose("getOrderLineDetails OUTPUT XML: "
						+ XMLUtil
								.extractStringFromDocument(getOrderLineDtlsOpDoc));
			}

			// Set an Element reference to getOrderLineDetails output document
			// element
			...
			// XML
			addLineToIssue(createOrderInput, currOrderLineFromUI,
		...
		if (logger.isVerboseEnabled() || icbsDebug.booleanValue()) {
			...
			logger.verbose("Change Order API Input XML:"
		...
		if (logger.isVerboseEnabled() || icbsDebug.booleanValue()) {
			logger.verbose("createIssuesFromBackOrderLines()->Create Order API Input XML -> "
					+ XMLUtil.extractStringFromDocument(createOrderInput));
		}
	...
	private Document createOrderHeader(YFSEnvironment env, Element orderLine)
		...
		String incidentNo = olOrderExtnElem
				.getAttribute(NWCGConstants.INCIDENT_NO);
		String incidentYear = olOrderExtnElem
				.getAttribute(NWCGConstants.INCIDENT_YEAR);

		if (logger.isVerboseEnabled())
			logger.verbose("Incident No/Year:" + incidentNo + "/"
					+ incidentYear);
		...
		String reqDeliveryDate = olOrderExtnElem
				.getAttribute("ExtnReqDeliveryDate");
		String shipAcctCode = olOrderExtnElem.getAttribute("ExtnShipAcctCode");
		String sAOverrideCode = olOrderExtnElem
				.getAttribute("ExtnSAOverrideCode");
		// Bottom of - CR 524 - populate extn attributes from OrderHeader
		// records
		if (!StringUtil.isEmpty(incidentNo)) {
			// -----------[Add the incident details]---------------//
			addIncidentDetails(env, createOrderInput, incidentNo, incidentYear,
					reqDeliveryDate, shipAcctCode, sAOverrideCode);
		}
		...
		if (logger.isVerboseEnabled() || icbsDebug.booleanValue()) {
			logger.verbose("Exiting createOrderHeader()-> "
					+ XMLUtil.extractStringFromDocument(createOrderInput));
	...
	private void addIncidentDetails(YFSEnvironment env,
			Document createOrderInput, String incidentNo, String incidentYear,
			String reqDeliveryDate, String shipAcctCode, String sAOverrideCode)
			throws Exception {
		Element createOrderInputDocElm = createOrderInput.getDocumentElement();
		...
		if (incidentYear.length() == 0)
			incidentYear = " ";
		getIncidentOrderInput.getDocumentElement().setAttribute(
				NWCGConstants.YEAR_ATTR, incidentYear);

		if (logger.isVerboseEnabled()) {
			String inputRightBeforeInvokeService = XMLUtil
					.extractStringFromDocument(getIncidentOrderInput);
			logger.verbose("Input to NWCGGetIncidentOrderService: "
					+ inputRightBeforeInvokeService);
		}

		Document getIncidentOrderServiceOpDoc = CommonUtilities.invokeService(
				env, NWCGConstants.NWCG_GET_INCIDENT_ORDER_SERVICE,
				getIncidentOrderInput);

		if (logger.isVerboseEnabled()) {
			String inputRightAfterInvokeService = XMLUtil
					.extractStringFromDocument(getIncidentOrderServiceOpDoc);
			logger.verbose("Output of NWCGGetIncidentOrderService: "
					+ inputRightAfterInvokeService);
		}
		...
		Element incidentDocElm = getIncidentOrderServiceOpDoc
				.getDocumentElement();

		// Copy the following attributes from the NWCGIncidentOrder node into
		// Order node
		// createOrderInputDocElm.setAttribute(NWCGConstants.SHIP_NODE,
		// incidentDocElm.getAttribute(NWCGConstants.PRIMARY_CACHE_ID));

		if (logger.isVerboseEnabled()) {
			logger.verbose("-----------------------------------------------------");
			logger.verbose("createOrder IP XML: "
					+ XMLUtil.extractStringFromDocument(createOrderInput));
			logger.verbose("-----------------------------------------------------");
		}

		Node olOrderExtnNode = XMLUtil.getChildNodeByName(
				createOrderInputDocElm, NWCGConstants.EXTN_ELEMENT);
		Element orderExtn = (olOrderExtnNode instanceof Element) ? (Element) olOrderExtnNode
				: null;

		// Top of CR 524 - populate Requested Delivery Date from the original
		// order
		if (orderExtn != null) {
			...
			if (!StringUtil.isEmpty(shipAcctCode)) {
				orderExtn.setAttribute("ExtnShipAcctCode", shipAcctCode); // user
																			// manually
																			// entered
																			// ShipAcctCode
			}
			if (!StringUtil.isEmpty(sAOverrideCode)) {
				orderExtn.setAttribute("ExtnSAOverrideCode", sAOverrideCode); // user
																				// manually
																				// entered
																				// ShipAcctOverrideCode
			}
		}
		// Bottom of CR 524 - populate Requested Delivery Date from the original
		// order
		// --------[Copy Incident addresses]--------------//

		// ShipTo
		Element incidentShipToAdd = (Element) XMLUtil.getChildNodeByName(
				incidentDocElm, "YFSPersonInfoShipTo");
		...
		Element incidentBillToAdd = (Element) XMLUtil.getChildNodeByName(
				incidentDocElm, "YFSPersonInfoBillTo");
		...
		Element incidentDeliverTo = (Element) XMLUtil.getChildNodeByName(
				incidentDocElm, "YFSPersonInfoDeliverTo");
		if (null != incidentDeliverTo) {
			Element additionalAddresses = createOrderInput
					.createElement("AdditionalAddresses");
			createOrderInputDocElm.appendChild(additionalAddresses);
			Element additionalAddress = createOrderInput
	 ...
	 * @throws Exception
	 */
	private Document addLineToIssue(Document createOrderInputDoc,
	...
	private Document changeBackOrderStatusInOriginalIssue(YFSEnvironment env,
		...
		if (origOLorderHeaderKey.equals(TempOrderHeaderKey)) {
			...
			logger.verbose("changeOrder API XML: (before): "
					+ XMLUtil.extractStringFromDocument(changeOrderInput));
			...
			orderLineExtn.setAttribute(NWCGConstants.EXTN_NAV_INFO_ATTR,
					NWCGConstants.SHIPPING_ADDRESS);
			logger.verbose("changeOrder API XML: (after): "
					+ XMLUtil.extractStringFromDocument(changeOrderInput));
		}
		...
		// NWCGConstants.API_CHANGE_ORDER,
		// changeOrderInput);
	...
	/**
	 * Calls and returns the output of the getOrderLineDetals API for the given
	 * orderLineKey
	 * 
	 * @param env
	 * @param orderLineKey
	...
	private Document getOrderLineDetails(YFSEnvironment env, String orderLineKey)
		...
		env.setApiTemplate(NWCGConstants.API_GET_ORDER_LINE_DETAILS,
				"NWCGCreateIssueFromBackOrderLines_getOrderLineDetails");
		Document returnDoc = CommonUtilities.invokeAPI(env,
	...
	/**
	 * Copies all attribute/value pairs from the 'from' Element to the 'to'
	 * Element
	 * 
	 * @param from
	...
	private Boolean icbsDebug = new Boolean(ResourceUtil.get(
			"icbs.devmode.debug", "false"));

...
public class NWCGSerialRecordProcessor {
	public ArrayList<NWCGTrackableItem> insertChildSerialNumbers(
			YFSEnvironment env, ArrayList<NWCGTrackableItem> serialList,
			String orderLineKey, String receiptLineKey,
			Map<String, String> baseMap) throws Exception {
		...
		for (NWCGTrackableItem trackItem : serialList) {
			...
			Document opTemplate = XMLUtil
					.getDocument("<SerialList><Serial GlobalSerialKey=\"\" SerialNo=\"\" /></SerialList>");
			Document ipDoc = XMLUtil.getDocument("<Serial SerialNo=\""
					+ serialNo + "\" />");
			Document intermediateDoc = CommonUtilities.invokeAPI(env,
					opTemplate, NWCGConstants.API_GET_SERIAL_LIST, ipDoc);

			String globalSerialKey = ((Element) intermediateDoc
					.getElementsByTagName("Serial").item(0))
					.getAttribute("GlobalSerialKey");
			Document childSerialsDoc = CommonUtilities.invokeAPI(
					env,
					NWCGConstants.API_GET_SERIAL_LIST,
					XMLUtil.getDocument("<Serial ParentSerialKey=\""
							+ globalSerialKey + "\" />"));
			...
			if (serialChildNodeList.getLength() > 0) {
				for (int j = 0; j < serialChildNodeList.getLength(); j++) {
					...
					insertRow(env, cItemId, childSerialNo, orderLineKey,
							receiptLineKey, trackItem.getItemId(),
							trackItem.getSerialNo(),
							NWCGTrackableItem.getNodeLevel(trackItem), baseMap);
					modSerialList.add(new NWCGTrackableItem(cItemId,
							childSerialNo, trackItem));
				}
				modSerialList.addAll(insertChildSerialNumbers(env,
						modSerialList, orderLineKey, receiptLineKey, baseMap));
			}
		}
	...
	private Document getTrackableItem(YFSEnvironment env, String serialNo)
			throws Exception {
		Document ipDoc = XMLUtil.getDocument("<NWCGTrackableItem SerialNo=\""
				+ serialNo + "\" />");
		return CommonUtilities.invokeService(env,
				NWCGConstants.NWCG_GET_TRACKABLE_ITEM_LIST_SERVICE, ipDoc);
	}

	public static Element getChildElementByName(Element parentElement,
			String childName) {
		for (Node child = parentElement.getFirstChild(); child != null; child = child
				.getNextSibling()) {
	...
	public static void insertRow(YFSEnvironment env, String itemId,
			String serialNo, String orderLineKey, String receiptLineKey,
			String parentItemID, String parentSerialNo, String nodeLevel,
			Map<String, String> baseMap) throws Exception {
		Document inDocForService = XMLUtil.getDocument();
		Element inDocRootElement = inDocForService
				.createElement("NWCGSerialRecord");

		if (baseMap != null) {
			inDocRootElement.setAttribute("OrderNo", baseMap.get("OrderNo"));
			inDocRootElement
					.setAttribute("ReceiptNo", baseMap.get("ReceiptNo"));
			inDocRootElement
					.setAttribute("OrderType", baseMap.get("OrderType"));
			inDocRootElement.setAttribute("DocumentType",
					baseMap.get("DocumentType"));
			inDocRootElement.setAttribute("TransactionType",
					baseMap.get("TransactionType"));
			inDocRootElement.setAttribute("IncidentNo",
					baseMap.get("IncidentNo"));
			inDocRootElement.setAttribute("IncidentYear",
					baseMap.get("IncidentYear"));
			inDocRootElement.setAttribute("OrderHeaderKey",
					baseMap.get("OrderHeaderKey"));
			inDocRootElement.setAttribute("ReceiptHeaderKey",
					baseMap.get("ReceiptHeaderKey"));
		...
		CommonUtilities.invokeService(env, "NWCGCreateSerialRecord",
				inDocForService);
	}
}
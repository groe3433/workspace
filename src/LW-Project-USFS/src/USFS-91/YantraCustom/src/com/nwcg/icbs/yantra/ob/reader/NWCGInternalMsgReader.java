...
public class NWCGInternalMsgReader implements YIFCustomApi {
	...
	private static YFCLogCategory logger = NWCGROSSLogger
			.instance(NWCGInternalMsgReader.class);
	...
	public Document processMsgFromQueue(YFSEnvironment env, Document docIP) throws Exception {
		logger.verbose("NWCGInternalMsgReader::process, Entered");
		logger.verbose("NWCGInternalMsgReader::process, Input document : " + XMLUtil.extractStringFromDocument(docIP));
		...
		if (docIP != null){
			...
			if (messageType != null && messageType.equalsIgnoreCase("CreateRequestAndPlaceReq")){
				...
				logger.verbose("NWCGInternalMsgReader::process, Unknown message");
			}
		}
		else {
	...
	private Document processCreateReqAndPlaceReqMessage(YFSEnvironment env, Document docIP){
		logger.verbose("NWCGInternalMsgReader::processCreateReqAndPlaceReqMessage, Entered");
		try {
			CommonUtilities.invokeService(env, "NWCGSendICBSRInitiatedRequestToROSSService", docIP);
		}
		catch(Exception e){
			logger.error("!!!!! NWCGInternalMsgReader::processCreateReqAndPlaceReqMessage, Exception : " + e.getMessage());

 ...
 */
public class NWCGUpdateNodeKeyOnkitComponents implements YIFCustomApi {
	
	private static YFCLogCategory log = NWCGApplicationLogger.instance(NWCGUpdateNodeKeyOnkitComponents.class);
	...
	public Document updateNodeKey(YFSEnvironment env, Document inXML)
			throws Exception {
		...
		if (lsReceiptLines != null) {
			for (int i = 0; i < lsReceiptLines.getLength(); i++) {
				...
				if (strSerialNo != null && !strSerialNo.equals("")) {
					...
					NodeList serialNodeList = opgetAllChildSerialComps
							.getDocumentElement()
					...
					if (serialNodeList != null) {
						for (int k = 0; k < serialNodeList.getLength(); k++) {
							Element SerialElem = (Element) serialNodeList
									.item(k);
							String CompSerialKey = SerialElem
		...
		if (lsSerialChilds != null && !lsSerialChilds.isEmpty()) {
			try {
				...
				while (itr.hasNext()) {
					...
					rs = stmt
							.executeQuery("update yfs_global_serial_num set shipnode_key = '"
									+ strNodeKey
									+ "'"
									+ " ,modifyts=sysdate,modifyprogid ='"
									+ strProgID
									+ "'"
	...
	private Document getAllChildSerialComps(YFSEnvironment env,
		...
		if (log.isVerboseEnabled())
			log.verbose("starting getAllChildSerialComponents ");
		...
		if (log.isVerboseEnabled())
			log.verbose("getAllChildSerialComponents::Invoking getSerialList with input "
					+ XMLUtil.getXMLString(docGetSerialListIP));
		docGetSerialListOP = CommonUtilities.invokeAPI(env, "getSerialList",
				docGetSerialListIP);
		if (log.isVerboseEnabled())
			log.verbose("getAllChildSerialComponents:: getSerialList output "
		...
		if (log.isVerboseEnabled())
			log.verbose("getAllChildSerialComponents got the golbal serial key ="
					+ strSerialKey);

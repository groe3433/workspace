...
<%
	...
	String issueType = resolveValue("xml:/Order/@OrderType");
...
//End CR845 02012013
		...
		for(i = 0 ; i < nodes.length  ; i++)
		{
			// this is used to set the order line special needs value for the first time
			if (i == 0){
				setSpecialInstructionsReqNo('');
			}
...
function window.onload()
{
	...
	if (numRowsToShow != null) {
		...
		if (getAvailRFIBool == 0) {
			//alert("nwcg_new_render_orderline_js.jsp::window.onload() 1a");
			...
			//alert("nwcg_new_render_orderline_js.jsp::window.onload() 1b");
			fetchDataWithParams(objNext,'NWCGGetSortedOrderLineList',populateOrderLines,setOrderLineParam('','<%=resolveValue("xml:/Order/@OrderHeaderKey")%>', numToDisplay));
		}
	}
	else {
		if (getAvailRFIBool == 0) {
			//alert("nwcg_new_render_orderline_js.jsp::window.onload() 1c");
		  ...
		  //alert("nwcg_new_render_orderline_js.jsp::window.onload() 1d");		
		fetchDataWithParams(objNext,'NWCGGetSortedOrderLineList',populateOrderLines,setOrderLineParam('','<%=resolveValue("xml:/Order/@OrderHeaderKey")%>','<%=maximumLinesToDisplay%>'));	
		}
	}
}
...
function addBlankRows(element)
{
	...
	{
		var row = populateBlankRow('<%=strDoc%>','<%=driverDate%>','<%=strPC%>','<%=strUOM%>','<%=issueType%>',i);
		//populateTable(row);
...
//End CR845 02012013
	...
	if (numRowsToShow != null) {
		var numToDisplay = numRowsToShow.value;
		if (getAvailRFIBool == 0) {
			//alert("nwcg_new_render_orderline_js.jsp::verifyPageNumberAndFetchData 2a");
			...
			//alert("nwcg_new_render_orderline_js.jsp::verifyPageNumberAndFetchData 2b");
			fetchDataWithParams(elem,'NWCGGetSortedOrderLineList',populateOrderLines,setOrderLineParam('','<%=resolveValue("xml:/Order/@OrderHeaderKey")%>', numToDisplay));
		}
	}
	else {
		if (getAvailRFIBool == 0) {
			//alert("nwcg_new_render_orderline_js.jsp::verifyPageNumberAndFetchData 2c");
			...
			//alert("nwcg_new_render_orderline_js.jsp::verifyPageNumberAndFetchData 2d");
			fetchDataWithParams(elem,'NWCGGetSortedOrderLineList',populateOrderLines,setOrderLineParam(elem,orderHeaderKey,maximumLinesToDisplay));
		}
	}	
}
...
function changeNoRecsToDisplay()
{
	...
	if (numSelected != null) {
		...
		document.getElementById("CurrentPageNumber").value = 1;	
		if (getAvailRFIBool == 0) {
		    //alert("nwcg_new_render_orderline_js.jsp::changeNoRecsToDisplay:: 3a");
			fetchDataWithParams(objNext,'getOrderLineListWithRFI',populateOrderLines,setOrderLineParam('','<%=resolveValue("xml:/Order/@OrderHeaderKey")%>', numToDisplay));
		}
		else {
		    //alert("nwcg_new_render_orderline_js.jsp::changeNoRecsToDisplay:: 3b");
			fetchDataWithParams(objNext,'NWCGGetSortedOrderLineList',populateOrderLines,setOrderLineParam('','<%=resolveValue("xml:/Order/@OrderHeaderKey")%>', numToDisplay));			
		}
	}
}

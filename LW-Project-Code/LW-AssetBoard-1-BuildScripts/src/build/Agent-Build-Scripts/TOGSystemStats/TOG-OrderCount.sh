...
then
	...
	echo "Entered TOGSystemStats Order Count..." >> /opt/ssfs/runtime/logs/TOGSystemStats.log;
	...
	echo $StdOut | sed -e 's/newLine /\n/g' -e 's/newLine//g' | mailx -s "TOGSystemStats Order Count" $MORE_DETAILED_EMAIL;

	echo "Exiting TOGSystemStats Order Count..." >> /opt/ssfs/runtime/logs/TOGSystemStats.log;

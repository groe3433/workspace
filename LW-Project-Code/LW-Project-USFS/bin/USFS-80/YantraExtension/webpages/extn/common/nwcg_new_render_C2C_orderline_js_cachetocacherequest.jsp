...
function populateOrderLines(elem,xmlDoc)
{
	...
	if(nodes!=null && nodes.length > 0 )
	{
		...
		for(i = 0 ; i < nodes.length  ; i++)
		{
			...
			populateSortableTable_CacheToCacheRequest(tableRow);
		}
	}
...
function addBlankRows_CacheToCacheRequest(element)
{
	var totalRows = '<%=strMax%>' * 1 ;
	for(i = 0 ; i < totalRows ; i++)
	{
		var row = populateBlankRow_CacheToCacheRequest('0006', '01', '<%=strPC%>','<%=strUOM%>',i);
		populateSortableTable_CacheToCacheRequest(row);
	}
...
function populateSortableTable_CacheToCacheRequest(tableRow)
{
	var tbl = document.getElementById("OrderLines");
	var lastRow = tbl.rows.length - 1;
	var row = tbl.insertRow(lastRow);

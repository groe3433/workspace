...
<thead>
    <tr> 
        ...
        <td sortable="no" class="tablecolumnheader" width="30px">
            <yfc:i18n>Trackable_ID_List</yfc:i18n>
        </td>
...
<%
		container.setAttribute("OrganizationCode", resolveValue("xml:/LocationInventory/InventoryItem/@OrganizationCode"));
		container.setAttribute("ItemID", resolveValue("xml:/LocationInventory/InventoryItem/@ItemID"));
		container.setAttribute("ProductClass", resolveValue("xml:/LocationInventory/InventoryItem/@ProductClass"));
		container.setAttribute("UnitOfMeasure", resolveValue("xml:/LocationInventory/InventoryItem/@UnitOfMeasure"));
		container.setAttribute("Node", resolveValue("xml:Inventory:/NodeInventory/@Node"));
...
%>
			...
			<td>
<%			
			String sSerialCapturedInInventory = getValue("Item", "xml:/Item/PrimaryInformation/@SerialCapturedInInventory");
			System.out.println("@@@@@ sSerialCapturedInInventory :: " + sSerialCapturedInInventory);
			String sTagCapturedInInventory = getValue("Item", "xml:/Item/@TagCapturedInInventory");
			System.out.println("@@@@@ sTagCapturedInInventory :: " + sTagCapturedInInventory);
			String sTimeSensitive = getValue("Item", "xml:/Item/InventoryParameters/@TimeSensitive");
			System.out.println("@@@@@ sTimeSensitive :: " + sTimeSensitive);
			String sIsSerialTracked = getValue("Item", "xml:/Item/InventoryParameters/@IsSerialTracked");
			System.out.println("@@@@@ sIsSerialTracked :: " + sIsSerialTracked);
			if(!YFCCommon.equals((String)request.getAttribute("SaveAllowed"), "N")) {
				bSerialTracked=false;			
				if((sSerialCapturedInInventory.equals("Y")) || (sTagCapturedInInventory.equals("Y")) || (sTagCapturedInInventory.equals("S")) || (sTimeSensitive.equals("Y"))) { 
					bSerialTracked=true;
%>
...
%>
			...
			<tr id='<%="optionSet_"+sCounter%>' class='<%=className%>' style="display:none">
				<td colspan="7" >
					<jsp:include page="/extn/wms/recordcount/detail/NWCG_count_detail_includetag.jsp" flush="true">
...
<%
			container.setAttribute("OrganizationCode", resolveValue("xml:/SummaryResultList/@OrganizationCode"));
			container.setAttribute("ItemID", resolveValue("xml:/SummaryResultList/@ItemID"));
			container.setAttribute("ProductClass", resolveValue("xml:/SummaryResultList/@ProductClass"));
			container.setAttribute("UnitOfMeasure", resolveValue("xml:/SummaryResultList/@UnitOfMeasure"));
			container.setAttribute("Node", resolveValue("xml:/SummaryResultList/CountRequest/@Node"));
			YFCElement elemItem = YFCDocument.createDocument("Item").getDocumentElement();
			request.setAttribute("Item",elemItem);
...
<%
			if(!YFCCommon.equals((String)request.getAttribute("SaveAllowed"),"N") ){
				bSerialTracked=false;			
				if(getValue("Item","xml:/Item/PrimaryInformation/@SerialCapturedInInventory").equals("Y") ||(getValue("Item","xml:/Item/@TagCapturedInInventory").equals("Y")) ||(getValue("Item","xml:/Item/@TagCapturedInInventory").equals("S")) ||(getValue("Item","xml:/Item/InventoryParameters/@TimeSensitive").equals("Y"))){ 
					bSerialTracked=true;
%>
				<img onclick="disableCountQty('<%=sCounter%>');expandCollapseDetails('optionSet_<%=sCounter%>', '<%=getI18N("Click_To_See_Tag_Info")%>','<%=getI18N("Click_To_Hide_Tag_Info")%>','<%=YFSUIBackendConsts.FOLDER_COLLAPSE%>','<%=YFSUIBackendConsts.FOLDER_EXPAND%>')"  style="cursor:hand" <%=getImageOptions(YFSUIBackendConsts.FOLDER,"Click_To_See_Tag_Info")%> />
<%
...
%>
			...
			<td class="numerictablecolumn">
				<input type="text" class="unprotectedinput" <%=getTextOptions("xml:/RecordCountResult/CountResult_"+sCounter+"/@CountQuantity", "xml:/CountResult/@CountQuantity")%>/>
<% 
    ...
    <tr style='display:none' TemplateRow="true">
		...
		<td class="tablecolumn" nowrap="true">
			 <select class="combobox" name="xml:/RecordCountResult/CountResult_/@OrganizationCode">
					<yfc:loopOptions binding="xml:EnterpriseList:/OrganizationList/@Organization" name="OrganizationCode" value="OrganizationCode"/>
			</select>
		</td>
		...
		<td nowrap="true" class="tablecolumn"  >
			<select class="combobox"  <%=getComboOptions("xml:/RecordCountResult/CountResult_/@ProductClass")%>>
				<yfc:loopOptions binding="xml:ProductClass:/CommonCodeList/@CommonCode" name="CodeValue" value="CodeValue" selected="xml:/RecordCountResult/CountResult_/@ProductClass"/>
			</select>
        </td>
		<td nowrap="true" class="tablecolumn">
		...
		<td nowrap="true" class="tablecolumn">
	       	<select name="xml:/RecordCountResult/CountResult_/@InventoryStatus" class="combobox" >
            	<yfc:loopOptions binding="xml:InventoryStatusList:/InventoryStatusList/@InventoryStatus" name="InventoryStatus" value="InventoryStatus" selected="xml:/RecordCountResult/CountResult_/@InventoryStatus"/>
		    </select>
        </td>
		<td/>

...
-->
<%
//System.out.println("Top of detail_linestoreceive.jsp");
...
if(equals("Y",strAfterSave)) {
	...
	if(oReceiptLines != null){
		for (Iterator i = oReceiptLines.getChildren(); i.hasNext();){
			...
			// --- iterates for each ReceiptLine --- //
			//String strRcvQty = oReceiptLine.getAttribute("Quantity");
			//YFCElement strExtn = oReceiptLine.getChildElement("Extn");
			//String strExtnRcvPrc = strExtn.getAttribute("ExtnReceivingPrice");
			/*
			xml:/Receipt/ReceiptLines/ReceiptLine_+i+/Extn/@ExtnReceivingPrice

			<input type="text" class="unprotectedinput" <%=getTextOptions("xml:/Receipt/ReceiptLines/ReceiptLine_" + ReceivableLineCounter+ "/Extn/@ExtnReceivingPrice",strUnitCost)
			*/
	...
	// - old: YFCElement oTempElement = YFCDocument.parse("<Receipt ReceiptHeaderKey=\"\" />").getDocumentElement(); 
...
<yfc:callAPI apiID="AP1"/>  <!--getLinesToReceive-->
...
<%
			...
			if(equals("Y",strAfterSave)) {
				...
				//System.out.println("ROOTTTTTTTTTTTT===>>> "+jroot );
				...
				//System.out.println("joReceiptLines===>"+joReceiptLines);
				if(joReceiptLines != null){
					for (Iterator i = joReceiptLines.getChildren(); i.hasNext();){
						...
						//System.out.println("rcvOrderLineKey===>["+rcvOrderLineKey+"]");
						String strRCVOrderLineKey = resolveValue("xml:/ReceivableLine/@OrderLineKey");
						//System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ strRCVOrderLineKey===>["+strRCVOrderLineKey+"]");
						if (rcvOrderLineKey != null && strRCVOrderLineKey != null && rcvOrderLineKey.equals(strRCVOrderLineKey)) {
							...
							if(jTagSerialElement != null) {
								...
								if(jExtnElement != null)
								{
									...
									//System.out.println("TagSerial -> extnRcvPrice===>["+extnRcvPrice+"]");
								}
							} else { // no TagSerial node
								YFCElement jExtnElement = joReceiptLine.getChildElement("Extn");
								if(jExtnElement != null)
								{
									extnRcvPrice = jExtnElement.getAttribute("ExtnReceivingPrice");
									//System.out.println("extnRcvPrice===>["+extnRcvPrice+"]");
								}
							}
						}
					} // for loop
			...
			//System.out.println("strUnitCost=["+strUnitCost+"]"); // comes from NWCG_supplier_item table...
...
<%				
			...
			//System.out.println("SuggestedDispositionCode = " + suggestedDispositionCode);
			//System.out.println("documentType = " + documentType);
			//System.out.println("orderType = " + orderType);
			//System.out.println("receivingPrice = " + receivingPrice);

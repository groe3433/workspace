...
<img onclick="expandCollapseDetails('<%=divToDisplay%>', '<%=getI18N("Click_To_See_Order_Lines")%>', '<%=getI18N("Click_To_Hide_Order_Lines")%>', '<%=YFSUIBackendConsts.FOLDER_COLLAPSE%>', '<%=YFSUIBackendConsts.FOLDER_EXPAND%>')" style="cursor:hand" <%=getImageOptions(YFSUIBackendConsts.FOLDER_COLLAPSE,"Click_To_Hide_Order_Lines")%> />

<%}%>
</td>
<td class="tablecolumn" style='width:150px'><yfc:i18n><yfc:getXMLValue binding="xml:/Order/@Relationship"/></yfc:i18n></td>
<td class="tablecolumn" style='width:150px'>
<a <%=getDetailHrefOptions("L02", getValue("Order", "xml:/Order/@DocumentType"), getParameter("orderKey"), "")%>>	<%=displayOrderStatus(getValue("Order","xml:/Order/@MultipleStatusesExist"),getValue("Order","xml:/Order/@MaxOrderStatusDesc"))%></a>

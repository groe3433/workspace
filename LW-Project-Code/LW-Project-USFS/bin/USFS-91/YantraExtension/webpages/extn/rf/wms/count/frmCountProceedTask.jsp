...
<%
	...
	if(!isVoid(sTaskId)){		
		...
		YFCElement oInElm = YFCDocument.parse("<Task TaskId=\""+sTaskId+"\" />", true).getDocumentElement();
		YFCElement tempForTask = YFCDocument.parse("<Task AssignedToUserId=\"\" TaskStatus=\"\"/>").getDocumentElement();
		
		%>
			<yfc:callAPI apiName="getTaskDetails" inputElement="<%=oInElm%>" templateElement="<%=tempForTask%>" outputNamespace="TaskDetails"/>
		<%
		
		String sTaskStatus = getValue("TaskDetails", "xml:/Task/@TaskStatus");
		String sAssignedUser = getValue("TaskDetails", "xml:/Task/@AssignedToUserId");

		if(equals("1100",sTaskStatus)){//accept task if it is in open status..

			YFCElement acceptInput = YFCDocument.parse("<TaskList UserId=\""+sUserId+"\" ><Task TaskId=\""+sTaskId+"\" /></TaskList>", true).getDocumentElement();
			%><yfc:callAPI apiName="acceptTasks" inputElement="<%=acceptInput%>" /> <%
			errorDesc=checkForError() ;

		}else if (isVoid(sAssignedUser)){ // assign user to task if not already assigned.

			YFCElement changeInput = YFCDocument.parse("<Task TaskId=\""+sTaskId+"\" AssignedToUserId=\""+sUserId+"\" InternalApp=\"Y\" />", true).getDocumentElement();
			%><yfc:callAPI apiName="changeTask" inputElement="<%=changeInput%>" /> <%
			errorDesc=checkForError() ;
		}
	} 
	...
	if(errorDesc == null) {
		...
		YFCElement taskListInput = YFCDocument.parse("<Task TaskId=\""+sTaskId+"\" AssignedToUserId=\""+sUserId+"\" TaskStatus=\"1300\" IsSummaryTask=\"N\"  TaskStatusQryType=\"LE\"><TaskType ActivityGroupId=\"COUNT\"/></Task>",true).getDocumentElement();

		YFCElement tempElem = YFCDocument.parse("<TaskList><Task><Inventory><TagAttributes><Extn/></TagAttributes></Inventory><TaskReferences/><TaskType/></Task></TaskList>").getDocumentElement();

			%>
			<yfc:callAPI apiName="getTaskList" inputElement="<%=taskListInput%>" templateElement="<%=tempElem%>" outputNamespace="TaskList"/>
	<%
		...
		if(errorDesc == null) {
				...
				if(equals("01",getValue("TaskList","xml:/TaskList/Task/TaskType/@SourceTaskSuggestion"))){
					...
					if(skipButton != null){
						...
						skipButton.setAttribute("url","/console/rfcount.ppc?action=frmCountTaskContext");
					}
				...
				if(equals("N", sForceLocation) || (!isVoid(sScannedLocationId))) {
					...
					if(locationScanElem!=null){
						...
						//CR-63274: Added the following code to hide the 2nd entry of Location ID.
						YFCElement	txtSugLocnId = getField(ydoc,"txtSugLocnId");
						if(txtSugLocnId!=null){
							txtSugLocnId.setAttribute("type","hidden");
							txtSugLocnId.setAttribute("subtype","Hidden");
						}
						//CR-63503: Added the following if() block to hide the Item ID label and value on count confirmation screen in case of Location Count.
						if (isVoid(resolveValue("xml:/TaskList/Task/Inventory/@ItemId")) && !hasItemClassification) {

...
// Display the selected order lines from the order lines inner panel of the issue details screen and paint 1 blank order line
...
function clearTableData() {
	...
	if(lastRow > 2 ) {
		...
		while (tbl.rows.length > 2) {
			// since the last row is the one with buttons deleting always the second last row
...
function populateBlankRowForConsolidationUI() {
	...
	if (driverDate == "02") { 
		strDate = strDate + '<input type="text" tabindex="7" OldValue="" style=\'width:60px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine/@ReqDeliveryDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/smcfs/console/icons/calendar.gif" />' ;
	} else { 
		strDate = strDate + '<input type="text" tabindex="7" OldValue="" style=\'width:60px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine/@ReqShipDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/smcfs/console/icons/calendar.gif"/> ' ;
	} 

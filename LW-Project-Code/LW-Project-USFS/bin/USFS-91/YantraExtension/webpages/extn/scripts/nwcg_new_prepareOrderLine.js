...
function prepareTableRow(orderLine,OrderLineCounter,documentType,driverDate,orderHeaderKey,strPrefix,strPostfix,strImage, drawSysCol) {
	...
	if(lnPriceTotal != null && lnPriceTotal.length > 0) {
		...
		strActPricQty = '<font size=2px>' + strActPricQty + '</font>';
	}
	var bModifyLineQty = true ; 
	// check the modification rules for the add quantity get the Modification Rules
	...
	if(lstModification  != null && lstModification.length > 0) {
		for(index = 0 ; index < lstModification.length ; index++) {
			...
			if(strModificationType == "ADD_QUANTITY") {
				...
				// and the user is allowed to change the value ONLY through over ride dont allow the user to modify the values
	...
	var strPrimeLink = "<font size=2px><A onclick='yfcShowDetailPopup(\"ISUYOMD170\", \"\", \"1010\", \"650\", new Object(), \"ISUorderline\", \"%3COrderLineDetail+OrderHeaderKey%3D%22"+orderHeaderKey+"%22+OrderLineKey%3D%22"+strLineKey+"%22%2F%3E\");return false;' href=\"\">"+strPrimeLineNo+"</A></font>";
	...
	if(elemTotalList != null && elemTotalList.length > 0 ) {
		...
		strTotal = '<font size=2px>' + strTotal + '</font>';
		totalObj.text = strTotal;
	...
	if(elemQtyList != null && elemQtyList.length > 0 ) {		
		...
		// END: Changed by Manish K to allow Trackable items to be UTF/Fwd/Back  and if the user can modify the line quantity
			if (condVarTwo != null && condVarTwo != '' && condVarTwo != ' ' && (condVarTwo  == 'XLD-Retrieval')) {
				strQty = '<td align="center" class="protectednumber"> <input align="center" style="width:40px; font-size:12px" class=numericprotectedinput value="'+nQty+'" OldValue="'+nQty+'" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/OrderLineTranQuantity/@OrderedQty" disabled="disabled"/>';
			}
			else if(nQty <= 0 || bModifyLineQty) {
				strQty = '<td> <input onBlur="checkIssuedQuantity(this)" style="width:40px; font-size:12px" class=numericunprotectedinput value="'+nQty+'" OldValue="'+nQty+'" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/OrderLineTranQuantity/@OrderedQty"/>';			
			} else {
				strQty = '<td> <input onBlur="checkIssuedQuantity(this)" style="width:40px; font-size:12px" class=numericunprotectedinput value="'+nQty+'" OldValue="'+nQty+'" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/OrderLineTranQuantity/@OrderedQty"/>';			
			}									
		} else {
			if (condVarTwo != null && condVarTwo != '' && condVarTwo != ' ' && (condVarTwo  == 'XLD-Retrieval')) {
				...
				strQty = '<td align="center"> <input align="center" style="width:40px; font-size:12px" class=numericprotectedinput value="'+nQty+'" OldValue="'+nQty+'" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/OrderLineTranQuantity/@OrderedQty"/>';			
			}
		}
	}
	...
	if(elemExtnList!=null && elemExtnList.length > 0 ) {
		...
		systemNo = '<font size=2px>' + systemNo + '</font>';
		strReqQty = '<td align="center"> <INPUT style="width:40px; font-size:12px" align="center" class=numericprotectedinput name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnOrigReqQty" value="' + strRequQty + '" dataType="STRING" OldValue="' + strRequQty +'" />';	
		// BEGIN: Changed by Manish K to allow Trackable items to be UTF/Fwd/Back
		if (draftOrCreatedFound == 1) {
		// END: Changed by Manish K to allow Trackable items to be UTF/Fwd/Back 
			strUTFQty  = '<td align="center"> <INPUT class=numericunprotectedinput '+disabledText+' name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnUTFQty" style="width:40px; font-size:12px"   value=\"' + strUTFQtyValue +'\" dataType="STRING"  OldValue=' + strUTFQtyValue +' />';
			strBackQty = '<td align="center"> <INPUT class=numericunprotectedinput '+disabledText+' name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnBackorderedQty" style="width:40px; font-size:12px" onBlur="setBackOrderFlag(this)"  value=\"'+ strBOQtyValue +'\" dataType="STRING"  OldValue=' + strBOQtyValue +' />';
			strFwdQty  = '<td align="center"> <INPUT onBlur="validateQuantity(this)" class=numericunprotectedinput '+disabledText+' name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnFwdQty" style="width:40px; font-size:12px"  onBlur="setForwardOrderFlag(this)"  value=\"' + strFwdQtyValue +'\" dataType="STRING" OldValue=' + strFwdQtyValue +' />';
		} else {
			strUTFQty  = '<td align="center"> <INPUT class=numericprotectedinput name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnBackorderedQty" style="width:40px; font-size:12px" value=\"' + strUTFQtyValue +'\" dataType="STRING" OldValue=' + strUTFQtyValue +' />';
			strBackQty = '<td align="center"> <INPUT class=numericprotectedinput name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnFwdQty" style="width:40px; font-size:12px" value=\"' + strBOQtyValue +'\" dataType="STRING" OldValue=' + strBOQtyValue +' />';
			strFwdQty = '<td align="center"> <INPUT onBlur="validateQuantity(this)" class=numericprotectedinput name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Extn/@ExtnFwdQty" style="width:40px; font-size:12px" value=\"' + strFwdQtyValue +'\" dataType="STRING" OldValue=' + strFwdQtyValue +' />';
		}
	...
	if (issueQty > 0) 
		strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">'+status+'</A></font>';			
	else
		strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Cancelled</A></font>';				
	if (condVarTwo != null && condVarTwo != '' && condVarTwo != ' ') {
		if (condVarTwo == 'XLD-Substituted') {
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Cancelled due to substitution</A></font>';
		} else if (condVarTwo == 'XLD-Consolidated' || condVarTwo == 'No-Cancellation') {
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Cancelled due to consolidation</A></font>';		
		} else if (condVarTwo == 'XLD-Retrieval') {
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Cancelled due to ROSS retrieval</A></font>';				
		}
	} else {
		var nQty = strUTFQtyValue * 1 ;
		if (nQty > 0 && issueQty == 0)
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">UTF</A></font>';
		var nQty = strFwdQtyValue * 1 ;
		if (nQty > 0 && issueQty == 0)
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Forwarded</A></font>';
		var nQty = strBOQtyValue * 1 ;
		if (strBOQtyValue > 0 && issueQty == 0)
			strLineStatus = '<TD class=protectedtext><font size=2px><A onclick=\''+strMethodCall+orderLine.getAttribute("OrderLineKey")+'%22%2F%3E");return false;'+'\' href="">Backordered</A></font>';
	}
	...
	if(elemNoteList != null && elemNoteList.length > 0 ) {
		...
		if (condVarTwo == 'XLD-Consolidated' || status == null || condVarTwo.indexOf("XLD") != -1) {
			strNote = '<td><INPUT type=text title="'+checkForNull(elemNote.getAttribute("NoteText"))+'" disabled="disabled" alt="'+ checkForNull(elemNote.getAttribute("NoteText")) +'" class=unprotectedinput style="width:100px; font-size:12px" value="' + checkForNull(elemNote.getAttribute("NoteText")) + '" name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Notes/Note/@NoteText" dataType="STRING" OldValue="'+checkForNull(elemNote.getAttribute("NoteText"))+'">';
		} else {
			strNote = '<td><INPUT type=text title="'+checkForNull(elemNote.getAttribute("NoteText"))+'" alt="'+ checkForNull(elemNote.getAttribute("NoteText")) +'" class=unprotectedinput +'+disabledText+' style="width:100px; font-size:12px" value="' + checkForNull(elemNote.getAttribute("NoteText")) + '" name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Notes/Note/@NoteText" dataType="STRING" OldValue="'+checkForNull(elemNote.getAttribute("NoteText"))+'">';
		}
	} else {
		strNote = '<td> <INPUT type=text class=unprotectedinput '+disabledText+' style="width:100px; font-size:12px" value="" name="xml:/Order/OrderLines/OrderLine_' + OrderLineCounter + '/Notes/Note/@NoteText" dataType="STRING" OldValue="">';		
	}
	var strItemID = '' ;
	var strProductClass = '' ;
	var strDesc = '' ;
	...
	if(elemItem!= null) {	
		...
		strItemIDCell = '<INPUT title="'+titleAttrib+'" class=protectedtext OldValue="'+strItemID+'" style="width:40px; font-size:12px" dataType="STRING" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/Item/@ItemID" value="'+strItemID+'"/>';
		...
		strProductClassCell = '<td><INPUT class=protectedtext OldValue="'+strProductClass+'" style="width:40px; font-size:12px" dataType="STRING" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/Item/@ProductClass" value="'+strProductClass+'"/>';
		strUOMCell = '<td><INPUT class=protectedtext OldValue="'+strUOM+'" style="width:15px; font-size:12px" dataType="STRING" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/Item/@UnitOfMeasure" value="'+strUOM+'"/>';
		...
		strDesc = '<font size=2px>' + strDesc + '</font>';
	...
	if(elemExtnList!=null && elemExtnList.length > 0 ) {
		...
		var strQtyRfiHidden = '<td class="tablecolumn" tabindex="-1"><INPUT type="hidden" style="width:40px; font-size:12px" tabindex="-1" name="xml:/Order/OrderLines/OrderLine_'+OrderLineCounter+'/Extn/@ExtnQtyRfi" value="' + rfiQty + '" dataType="STRING"/></td>';	
		strQtyRfi = '<td class=protectedtext><font size=2px><a onclick=\'' + rfiMethodCall + '\' href="">' + rfiQty + '</a></font></td>' + strQtyRfiHidden;
	}
...
function populateBlankRow(documentType,driverDate,strPCData,strUOMData,issueType,counter) {
	...
	var requestNoObj = new Object();
	var sysNo = '<td><INPUT class=unprotectedinput OldValue=""  name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnSystemNo" value="" maxLength=10 size=12 dataType="STRING"/>' ;
	if (ExtnRequestNo != null) {
        ExtnRequestNo.focus();
    }
	if(!bOtherIssues) {
		requestNoCol = '<INPUT class=unprotectedinput OldValue="S-"  name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnRequestNo"  onblur="validateRequestNumber(this)"   value="S-" maxLength=20 size=12 dataType="STRING"/>' ;
		requestNoObj.text = requestNoCol;
		requestNoObj.className = "tablecolumn";	
	} else {
		requestNoCol = '<INPUT class=unprotectedinput  name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnRequestNo" maxLength=20 size=12 dataType="STRING"/>' ;
		requestNoObj.text = requestNoCol;
		requestNoObj.className = "tablecolumn";	
	}
	//Begin CR833
	var idItemID = "itemid"+counter;
	var strItemID = '<td class="tablecolumn" style="width:50px" nowrap="true"> <input class=unprotectedinput type="text" style="width:40px; font-size:12px" id="'+idItemID+'" dataType="STRING" name="xml:/Order/OrderLines/OrderLine_'+counter+'/Item/@ItemID" ';
	//End CR833
    ...
    //Begin CR833
	strItemID= strItemID +  'onclick="templateRowCallItemLookup(this,\'ItemID\',\'ProductClass\',\'TransactionalUOM\',\'item\',\'<%=extraParams%>\');returnFocusToItemId(\''+counter+'\');" src=\'/smcfs/console/icons/lookup.gif\'/>' ;
	//End CR833
	var strProductClass = '<td class="tablecolumn"><INPUT class=unprotectedinput readonly="true" tabindex="-1" style="width:40px; font-size:12px"  name="xml:/Order/OrderLines/OrderLine_'+counter+'/Item/@ProductClass" dataType="STRING">'; 
	var strUOM = '<td class="tablecolumn"><INPUT class=unprotectedinput readonly="true" tabindex="-1" style="width:40px; font-size:12px"  name="xml:/Order/OrderLines/OrderLine_'+counter+'/OrderLineTranQuantity/@TransactionalUOM" dataType="STRING">'; 
	//fixing line display here var strDesc = '<td class="tablecolumn" style="width:120px"></td>' ;
	var strDesc = '<td class="tablecolumn" style="width:120px"><label style="width:140px; font-size:12px" value=""/></td>' ;
	var strReqQty = '<td class="tablecolumn"> <INPUT class=unprotectedinput onBlur="checkMaxQty(this)" maxLength=10 size=5 style="width:40px; font-size:12px" name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnOrigReqQty" dataType="STRING">' ;
	var strQtyRfiHidden = '<td class="tablecolumn" tabindex="-1"><INPUT type="hidden" class=protectedinput style="width:40px; font-size:12px" tabindex="-1" title="Currently Available RFI Quantity\rClick for details." name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnQtyRfi" dataType="STRING"/></td>';	
	//fixing font here (size=2 equals fontSize=11)
	var strQtyRfi = '<td class=protectedtext><font size=2px><a id="availRFIQty" tabindex="-1" href=""></a></font></td>' + strQtyRfiHidden;
	var strQty = '<td class="numerictablecolumn" > <input onBlur="checkIssuedQuantity(this);checkMaxQty(this)" style="width:40px; font-size:12px" type="text" class=unprotectedinput	name="xml:/Order/OrderLines/OrderLine_'+counter+'/OrderLineTranQuantity/@OrderedQty"/>';
	var strUTFQty = '<td class="tablecolumn"> <INPUT class=unprotectedinput style=\'width:40px; font-size:12px\' name=\'xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnUTFQty\' dataType="STRING">';
	var strBackQty = '<td class="tablecolumn"><INPUT onBlur="setBackOrderFlag(this);checkMaxQty(this)"  class=unprotectedinput style=\'width:40px; font-size:12px\' name=\'xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnBackorderedQty\' dataType="STRING" >';
	...
	var strFwdQty = '<td class="tablecolumn"><input type="text" onBlur="validateQuantity(this);checkMaxQty(this)" class=\'unprotectedoverrideinput\' style=\'width:40px; font-size:12px\' name="xml:/Order/OrderLines/OrderLine_'+counter+'/Extn/@ExtnFwdQty"/>'; 
	...
	var strDate = '<td class="tablecolumn" nowrap="true"> ' ;
	if (driverDate == "02") { 
		strDate = strDate + '<input type="text" OldValue="" style=\'width:60px; font-size:12px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine_'+counter+'/@ReqDeliveryDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/smcfs/console/icons/calendar.gif" />' ;
	} else { 
		strDate = strDate + '<input type="text" OldValue="" style=\'width:60px; font-size:12px\' class=\'unprotectedoverrideinput\'  name="xml:/Order/OrderLines/OrderLine_'+counter+'/@ReqShipDate"/> <img class="lookupicon" onclick="invokeCalendar(this)" src="/smcfs/console/icons/calendar.gif"/> ' ;
	} 
	strDate = strDate + '</td> ' ;
	var strNote = '<td> <INPUT class=unprotectedinput  style=\'width:100px; font-size:12px\' name="xml:/Order/OrderLines/OrderLine_'+counter+'/Notes/Note/@NoteText" dataType="STRING" OldValue=""/>';
	var strActPricingQty = '<td class="tablecolumn" style="width:30px"><font size=2px><label style="width:30px; font-size:12px" value=""/></font>';
	arr.push(strChkBox);
	arr.push(strBlank);
	arr.push(requestNoObj) ;	
	arr.push(strItemID );

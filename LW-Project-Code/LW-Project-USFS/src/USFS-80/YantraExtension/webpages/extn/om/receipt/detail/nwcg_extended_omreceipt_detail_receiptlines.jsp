			...
			for (Iterator i = oItem.getChildren(); i.hasNext();)
				{
					...
					oRep.getChildElement("Extn");
					YFCElement extn = oRep.getChildElement("Extn");
											...
											+checkNull(oRep.getAttribute("ReleaseNo"))
												+checkNull(oRep.getAttribute("TagNumber"))
													+checkNull(oRep.getAttribute("PrimeLineNo"))
														+checkNull(extn.getAttribute("ReceivingPrice"));
...
<thead>
    <tr>  
		...
		<td class="tablecolumnheader" style="width:140px">
            <yfc:i18n>Description</yfc:i18n>
        </td>
        <td class="tablecolumnheader" style="width:40px">
		...
		<td class="tablecolumnheader" style="width:<%= getUITableSize("xml:/Receipt/ReceiptLines/ReceiptLine/Extn/@ReceivingPrice")%>">
...
<tbody>
	...
	<% 
		if (!isVoid(resolveValue("xml:/ReceiptLine/@ItemID"))) {

        sKey = resolveValue("xml:/ReceiptLine/@ItemID")
                        +resolveValue("xml:/ReceiptLine/@ProductClass")
                                    ...
                                    +resolveValue("xml:/ReceiptLine/@ReleaseNo")
                                        +resolveValue("xml:/ReceiptLine/@TagNumber")
                                            +(int)getNumericValue("xml:/ReceiptLine/@PrimeLineNo")
												+ resolveValue("xml:/ReceiptLine/Extn/@ReceivingPrice");
		//System.out.println("\n************************\n oMap ==> "+oMap);
...
<%						}	%>                        
					...
					<td class="numerictablecolumn">
					<yfc:getXMLValue name="ReceiptLine" binding="xml:/ReceiptLine/Extn/@ExtnReceivingPrice"/>

...
<tr>
    <% if (equals("true", showDocumentType)) {
        ...
        String excludeCTDocumentType = getParameter("ExcludeCTDocumentType");
        if (isVoid(excludeCTDocumentType)) {
            // The default value of ExcludeCTDocumentType is "false"
            excludeCTDocumentType = "false";
        }
                ...
                <%=getComboOptions(documentTypeBinding)%>>
                    <% if (null != allDocTypeListElement) {
                        for (Iterator i = allDocTypeListElement.getChildren(); i.hasNext();) {
							...
							<% if (equals(excludeCTDocumentType, "true")) {
								if (!equals("0006", singleDocType.getAttribute("DocumentType"))) { %>
    ...
    <% }
        ...
        if ((equals("true", acrossAllNodesAllowed)) && (!equals("false", request.getParameter("AcrossNodes")))) {
            ...
            <% 
               ...
               // System.out.println("Total number of nodes is " + String.valueOf(numberOfNodes));

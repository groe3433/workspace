...
public class NWCGCancelBOFromBackOrderLines implements YIFCustomApi {
	...
	private static Logger logger = Logger
			.getLogger(NWCGCancelBOFromBackOrderLines.class.getName());
	...
	/**
	 * @param env
	 * @param inDoc The orderlines with the backorder qty
	 * @return Document The createOrder output after the issue has been created 
	 * @throws Exception
	 */
	public Document cancelBOFromBackOrderLines(YFSEnvironment env, Document inDoc) throws Exception{
		...
		// validate the lines before processing. if the input is not valid, then throw exception 
		...
		for(int i=0; i<inpOlList.getLength(); i++){
			...
			if(i==0){
				...
				createIssueInput = createIssueHeader(env,orderLine); // new Issue header is created
			}
			//add the line to the create order input
			output_doc = addLineToIssue(createIssueInput,ol,orderLine);
			changeOrderInput = changeBackOrderStatusInOriginalIssue(env,ol,orderLine,changeOrderInput);
			if(logger.isVerboseEnabled())
				logger.verbose("ChangeOrder API Input XML ->\n"+ XMLUtil.getXMLString(changeOrderInput));
			returnDoc = CommonUtilities.invokeAPI(env,"changeOrder",changeOrderInput);
		} //--- for loop ---//
		...
		returnDoc = CommonUtilities.invokeAPI(env,createOrderTemplate,"createOrder",createIssueInput);

		if(logger.isVerboseEnabled())
			logger.verbose("After CreateOrderAPI OutPut XML ->\n"+ XMLUtil.getXMLString(returnDoc));

		// extract newly created orderHeaderKey
		String createdOrderHeaderKey = returnDoc.getDocumentElement().getAttribute("OrderHeaderKey");
		if(logger.isVerboseEnabled())
		...
		//<ConfirmDraftOrder IgnoreOrdering="Y" OrderHeaderKey="200811071118071279097"/>
		// --> set up all the attributes
		Document confirmDraftOrderInput = XMLUtil.createDocument("ConfirmDraftOrder");
		Element elemconfirmDraftOrderInput = confirmDraftOrderInput.getDocumentElement();
		elemconfirmDraftOrderInput.setAttribute("IgnoreOrdering","Y");
		elemconfirmDraftOrderInput.setAttribute("OrderHeaderKey",createdOrderHeaderKey);
		if(logger.isVerboseEnabled())
			logger.verbose("cancelBOFromBackOrderLines()->confirmDraftOrder API Input XML ->\n"+ XMLUtil.getXMLString(confirmDraftOrderInput));
		returnDoc = CommonUtilities.invokeAPI(env,"confirmDraftOrder",confirmDraftOrderInput);
		...
		elemchgordinput.setAttribute("ModificationReasonCode","CUSTOMERCHANGE");
	...
	/*
	 * This method validates the input from the JSP
	 * if the selected lines belong to different order/incidents then we throw an exception
	 * if the issue quantity for the new lines is not entered or not a number we throw and exception
	 */
	private void validateLines(YFSEnvironment env, Document inDoc) throws Exception{
		...
		for(int i=0;i<inpOlList.getLength();i++){
			Element ol = (Element)inpOlList.item(i);
			//---[Check the primary order identifier - IncidentNo+Year or IncidentNo]-----//
			String tempOrderIdentifier = ol.getAttribute("IncidentNo");
			String tempIncidentYear = ol.getAttribute("IncidentYear");
			if(logger.isVerboseEnabled())
				logger.verbose("incidentNo=["+tempOrderIdentifier+"] tempIncidentYear=["+tempIncidentYear+"]");
			if(StringUtil.isEmpty(tempOrderIdentifier))
				tempOrderIdentifier = NWCGConstants.PRIMARY_IDENTIFIER_NULL;

			// check if the IncidentNo/Year or Other Order No are the same for all passed order lines
			if(!StringUtil.isEmpty(tempIncidentYear)) { // Incident Issue cases
				if(i==0){
					// for the first line assign the identifier
					primaryOrderIdentifier = tempOrderIdentifier; // save the first incidentNo
					primaryIncidentYear = tempIncidentYear;       // save the first incidentYear
				}else{
					//Check if the identifiers are not the same and if they differ, throw an exception 
					if(!primaryOrderIdentifier.equals(tempOrderIdentifier) || !primaryIncidentYear.equals(tempIncidentYear))
				...
				if(i==0){
					...
					//Check if the identifiers are not the same and throw and exception 
			...
			if(logger.isVerboseEnabled())
				logger.verbose("primaryOrderIdentifier=["+primaryOrderIdentifier+"] primaryIncidentYear=["+primaryIncidentYear+"]");
			...
			if(StringUtil.isEmpty(newIssueQty)){
				...
				try{
					...
					throw new NWCGException("NWCG_ISSUE_BACKORDER_004",new String[]{newIssueQty});
				}
	...
	/**
	 * Create the issue header using the original order details
	 * also incorporate the incident details into the header from the original order 
	 * @param env
	 * @param orderLine
	 * @return
	 * @throws Exception
	 */
	private Document createIssueHeader (YFSEnvironment env,Element orderLine ) throws Exception{
		...
		Element olOrdElem = (Element)XMLUtil.getChildNodeByName(orderLine,"Order");
		order.setAttribute("DraftOrderFlag","Y");//set the draft order flag 
		order.setAttribute("EnterpriseCode",olOrdElem.getAttribute("EnterpriseCode"));
		order.setAttribute("SellerOrganizationCode",olOrdElem.getAttribute("SellerOrganizationCode"));
		order.setAttribute("BillToID",olOrdElem.getAttribute("BillToID"));
		order.setAttribute("DocumentType",olOrdElem.getAttribute("DocumentType"));
		order.setAttribute("OrderType","Backordered");
		order.setAttribute("ShipNode",olOrdElem.getAttribute("ShipNode"));
		//---[end set headers]--//
		
		Element olOrdExtnElem = (Element)XMLUtil.getChildNodeByName(olOrdElem,"Extn");
	...
	/**
	 * This method adds the incident details to the order header if Incident no is not null
	 * @param env
	 * @param createIssueInput
	 * @param incidentNo
	 * @throws Exception
	 */
	private void addIncidentDetails(YFSEnvironment env,Document createIssueInput,String incidentNo,String year) throws Exception{
		Element order = createIssueInput.getDocumentElement();
		...
		Document incidentDetails = CommonUtilities.invokeService(
		...
		//copy the following attributes from the NWCGIncidentOrder node into Order node
		...
		orderExtn.setAttribute("ExtnUnitType",incident.getAttribute("UnitType"));
		orderExtn.setAttribute("ExtnDepartment",incident.getAttribute("Department"));
		...
		orderExtn.setAttribute("ExtnIncidentNo",incident.getAttribute("IncidentNo"));
		orderExtn.setAttribute("ExtnIncidentYear",incident.getAttribute("Year"));
		orderExtn.setAttribute("ExtnIncidentName",incident.getAttribute("IncidentName"));
		orderExtn.setAttribute("ExtnFsAcctCode",incident.getAttribute("IncidentFsAcctCode"));
		orderExtn.setAttribute("ExtnBlmAcctCode",incident.getAttribute("IncidentBlmAcctCode"));
		orderExtn.setAttribute("ExtnOtherAcctCode",incident.getAttribute("IncidentOtherAcctCode"));
		orderExtn.setAttribute("ExtnOverrideCode",incident.getAttribute("OverrideCode"));

		//added the below three attributes
		orderExtn.setAttribute("ExtnCustomerName",incident.getAttribute("CustomerName"));
		orderExtn.setAttribute("ExtnShipAcctCode",incident.getAttribute("IncidentFsAcctCode"));
		orderExtn.setAttribute("ExtnSAOverrideCode",incident.getAttribute("OverrideCode"));

		orderExtn.setAttribute("ExtnIncidentTeamType",incident.getAttribute("IncidentTeamType"));
		orderExtn.setAttribute("ExtnIncidentType",incident.getAttribute("IncidentType"));
		orderExtn.setAttribute("ExtnPoNo",incident.getAttribute("CustomerPONo"));
		
		//--------[copy incident addresses]--------------//
		//shipto

		Element incidentShipToAdd = (Element)XMLUtil.getChildNodeByName(incident,"YFSPersonInfoShipTo");
		...
		//billto
		Element incidentBillToAdd = (Element)XMLUtil.getChildNodeByName(incident,"YFSPersonInfoBillTo");
		...
		//deliverto
		Element incidentDeliverTo =(Element)XMLUtil.getChildNodeByName(incident,"YFSPersonInfoDeliverTo");
	...
	/**
	 * @param from Address element
	 * @param to Address element
	 */
	...
	/**
	 * This method adds and orderline to the create issue input
	 * It gets the issue qty from the input orderline and adjusts it with the order qty while creating the new issue
	 * It also sets the backorder flag on the line if the issue qty cannot does not fully satisfy the backorder qty
	 * The S-Number or the request no. is also updated with increments/extensions eg. S_1 would become S_1.1 
	 * after first extension and S_1.2 after second extension and so on.   
	 * @param orderDoc
	 * @param inOrderLine
	 * @param origOrderLine
	 * @return Document
	 * @throws Exception
	 */
	private Document addLineToIssue(Document orderDoc,Element inOrderLine,Element origOrderLine) throws Exception{
        if(logger.isVerboseEnabled())
		 	logger.verbose("in addLineToIssue() orderDoc XML ->\n"+XMLUtil.getXMLString(orderDoc));
		Element orderLines = (Element)XMLUtil.getChildNodeByName(orderDoc.getDocumentElement(),"OrderLines");
		...
		Element olExtn = (Element)XMLUtil.getChildNodeByName(orderLine,"Extn");
		Element origOlExtn = (Element)XMLUtil.getChildNodeByName(origOrderLine,"Extn");
		String origBackOrderQty = origOlExtn.getAttribute("ExtnBackorderedQty");
		// if neither strings are empty
		if(!StringUtil.isEmpty(newIssueQty) && !StringUtil.isEmpty(origBackOrderQty) ){
			...
			if(delta >0 || delta == 0){ //set the difference as the back order qty in the new order
				olExtn.setAttribute("ExtnBackorderedQty",Double.toString(delta));
				olExtn.setAttribute("ExtnOrigReqQty",origBackOrderQty);// set the RequestedQty
				olExtn.setAttribute("ExtnBackOrderFlag","Y");// set the back order flag to Y
				if(delta == 0){ //if entire backorderedqty is consumed, i.e. become issue qty
					olExtn.setAttribute("ExtnBackOrderFlag","N");// set the back order flag to N 
				}
			}
		}
		...
		if(!StringUtil.isEmpty(requestNo)){
			String parts[] = requestNo.split("\\"+NWCGConstants.REQUEST_NO_EXTN_DELIM);
			...
			requestNo = parts[0]+NWCGConstants.REQUEST_NO_EXTN_DELIM+revision;
	...
	/**
	 * This method removes the backorder status from the original line as all the backorder qty is already transfered to the
	 * new issue
	 * @param env
	 * @param inpOrderLine
	 * @param origOrderLine
	 ...
	 * @throws Exception
	 */
	private Document changeBackOrderStatusInOriginalIssue(YFSEnvironment env,Element inpOrderLine ,Element origOrderLine,Document changeOrderInput)throws Exception{
		...
		{
			Element order = changeOrderInput.getDocumentElement();
			Element orderLines = (Element) order.getElementsByTagName("OrderLines").item(0);
			Element orderLine = changeOrderInput.createElement("OrderLine");
			orderLines.appendChild(orderLine);
			orderLine.setAttribute("OrderLineKey",inpOrderLine.getAttribute("OrderLineKey"));
		...
		{
			//System.out.println("CO API XML "+ XMLUtil.getXMLString(changeOrderInput));
    		returnDoc = CommonUtilities.invokeAPI(env,"changeOrder",changeOrderInput);
    		changeOrderInput = XMLUtil.createDocument("Order");
    		Element order = changeOrderInput.getDocumentElement();
			order.setAttribute("Override","Y");
			order.setAttribute("OrderHeaderKey",origOrderLine.getAttribute("OrderHeaderKey"));
			Element orderLines = changeOrderInput.createElement("OrderLines");
			order.appendChild(orderLines);
			Element orderLine = changeOrderInput.createElement("OrderLine");
			orderLines.appendChild(orderLine);
		...
		//Document returnDoc = CommonUtilities.invokeAPI(env,"changeOrder",changeOrderInput);
		TempOrderHeaderKey = OrderHeaderKey; 
	...
	private Document getOrderLineDetails(YFSEnvironment env,String orderLineKey) throws Exception{
		...
		getOrderLineDetailsInput.getDocumentElement().setAttribute("OrderLineKey",orderLineKey);
		env.setApiTemplate("getOrderLineDetails","NWCGCancelBOFromBackOrderLines_getOrderLineDetails");

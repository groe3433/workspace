...
{
	public Document createCacheTransferFromBackOrderLines(YFSEnvironment env, Document inDoc) throws Exception
	{
		//System.out.println("NWCGCreateCacheTransferFromBackOrderLines:createCacheTransferFromBackOrderLines:Input XML\n" + XMLUtil.getXMLString(inDoc));
		...
		if (inputDocElm == null) {
			NWCGException e = new NWCGException("NWCG_CACHE_TRANSFER_BACKORDER_000");
			throw e;
		}
		...
		for (int i = 0; i < inputDocOrderLineList.getLength(); i++) 
		{
			...
			addCacheTransferLine(createOrderInput, currOrderLineFromUI, getOrderLineDtlsDocElm, i+1);
		...
		//System.out.println("NWCGCreateCacheTransferFromBackOrderLines:createCacheTransferFromBackOrderLines:createOrderInput XML\n" + XMLUtil.getXMLString(createOrderInput));
	...
	private void validateLines(YFSEnvironment env, Document inDoc) throws Exception 
	{
		...
		for (int i = 0; i < inputDocOrderLineList.getLength(); i++) {
			...
			if (i == 0) {
				...
				// Check if the identifiers are not the same and throw an exception
			...
			if (StringUtil.isEmpty(newIssueQty)) {
				...
				try {
					...
					throw new NWCGException("NWCG_CACHE_TRANSFER_BACKORDER_004",
	...
	private Document createOrderHeader (YFSEnvironment env, Element orderLine ) throws Exception 
	{
		...
		createOrderInputDocElm.setAttribute(NWCGConstants.RECEIVING_NODE, orderLine.getAttribute(NWCGConstants.RECEIVING_NODE));
		...
		Element orderDatesElement = createOrderInput.createElement("OrderDates");
		...
		orderDateElement.setAttribute("ExpectedDate", formatDate.format(currentDate.getTime()));		
		...
		Element olShipToElem = (Element) XMLUtil.getChildNodeByName(orderLine, "PersonInfoShipTo");
		...
		else
		{
		    ...
		    Element organizationElement = inDocOrgList.createElement("Organization");
		    inDocOrgList.appendChild(organizationElement);
		    organizationElement.setAttribute("OrganizationCode", orderLine.getAttribute(NWCGConstants.RECEIVING_NODE));
		    
  	        Document outDocOrgList = CommonUtilities.invokeAPI(env, orgListTemplate, "getOrganizationList", inDocOrgList);
  	        Element billingPersonInfoElement = (Element) outDocOrgList.getElementsByTagName("BillingPersonInfo").item(0);
	...
	private Document addCacheTransferLine(Document createOrderInputDoc, Element inOrderLine, Element origOrderLine, int lineCounter) 
	throws Exception 
	{
		...
		// Set the Order/Extn/@ExtnShippingContactName&Phone from the original OrderLine/Order/Extn element
		orderLineExtn.setAttribute(NWCGConstants.SHIP_CONTACT_NAME_ATTR , origOrderLineExtn.getAttribute(NWCGConstants.SHIP_CONTACT_PHONE_ATTR));
		orderLineExtn.setAttribute(NWCGConstants.SHIP_CONTACT_PHONE_ATTR, origOrderLineExtn.getAttribute(NWCGConstants.SHIP_CONTACT_PHONE_ATTR));
	...
	private Document changeBackOrderStatusInOriginalIssue(YFSEnvironment env,Element inpOrderLine, Element origOrderLine, Document changeOrderInput) 
	throws Exception 
	{
	...
	private Document getOrderLineDetails(YFSEnvironment env, String orderLineKey) throws Exception{
		...
		env.setApiTemplate(NWCGConstants.API_GET_ORDER_LINE_DETAILS, "NWCGCreateCacheTransferFromBackOrderLines_getOrderLineDetails");
		...
		//System.out.println("NWCGCreateCacheTransferFromBackOrderLines:getOrderLineDetails:OrderLineDetails XML\n" + XMLUtil.getXMLString(returnDoc));

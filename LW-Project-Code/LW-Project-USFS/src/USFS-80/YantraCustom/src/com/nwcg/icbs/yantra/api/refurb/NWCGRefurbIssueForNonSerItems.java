...
public class NWCGRefurbIssueForNonSerItems implements YIFCustomApi{
  ...
  throws NWCGException,Exception {
  	try {
  		log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Entered");
  		if (log.isVerboseEnabled()){
  			log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, inputDoc : " + XMLUtil.getXMLString(inputDoc));
  		}
    	Element rootNode = inputDoc.getDocumentElement();
    	String qtyCmpl = XMLUtil.getAttribute(rootNode, "QuantityCompleted");;
    	String qtyReq  = XMLUtil.getAttribute(rootNode, "QuantityRequested");
    	log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Qty Cmpl : " + qtyCmpl + ", Qty Req : " + qtyReq);
    	if (qtyCmpl.equalsIgnoreCase(qtyReq)){
    		log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Creating Input Refurb Issue Doc");
    		...
    		log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Invoking Create Order : " + NWCGConstants.API_CREATE_ORDER);
    		
    		if(inputRfbIssueDoc != null )
    		{
    			rfbIssueOutputDoc = CommonUtilities.invokeAPI(
    														env, "NWCGRefurbIssue_createOrder", NWCGConstants.API_CREATE_ORDER, inputRfbIssueDoc);
    		}
     		
    		if (rfbIssueOutputDoc != null){
      		// Update the workorder on creation of workorder.
    			NWCGRefurbWOCost refurbWOCost = new NWCGRefurbWOCost();
    			log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Updating Refurb WO Cost");
    			Document updtWOPrice = refurbWOCost.updateWORefurbCost(env, rfbIssueOutputDoc);
    			log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Creating input status doc");
      		Document chgStatusDoc = createChgIssueStatusDoc(rfbIssueOutputDoc);
      		// Changing the status to an extended status
      		log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Changing the status : " + XMLUtil.getXMLString(chgStatusDoc));
      		Document chgStatusOutput = CommonUtilities.invokeAPI(env, NWCGConstants.API_CHANGE_ORDER_STATUS, chgStatusDoc);      		
      		log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Changed the status");
    		}
    		else {
    			log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Unable to create refurb issue as order document is NULL ");
    		}
    	}
    	else {
      	log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Quantity Completed and Requested are different");
      	//throw new NWCGException("NWCG_RFB_CREATE_ISSUE_001", new String[] {qtyCmpl, qtyReq});
    	}
  	}
  	...
  	log.verbose("NWCGRefurbIssueForNonSerItems::checkAndCreateRefurbIssue, Exiting");
  	return inputDoc;
  }
  ...
  throws NWCGException{
  	try {
  		log.verbose("NWCGRefurbIssueForNonSerItems::createRefurbIssueDocument, Entered");
    	...
    	String noOfUnits = XMLUtil.getAttribute(rootNode, "QuantityRequested");
    	...
    	NodeList orderLinesList = XPathUtil.getNodeList(rootNode, 
    	...
    	if (log.isVerboseEnabled()){
      	log.verbose("NWCGRefurbIssueForNonSerItems::createRefurbIssueDocument, Order XML : " + XMLUtil.getXMLString(orderIssueDoc));
    	}
    	log.verbose("NWCGRefurbIssueForNonSerItems::createRefurbIssueDocument, Returning");
	...
	private Hashtable getIncidentDetails(YFSEnvironment env, String workOrderKey){
		...
		try {
			...
			if (log.isVerboseEnabled()){
				log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentDetails, Input XML : " + XMLUtil.getXMLString(inputDoc));
			}
			...
			if (outputDoc != null){
				if (log.isVerboseEnabled()){
					log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentDetails, getWorkOrderDetails : " + XMLUtil.getXMLString(outputDoc));
				}
	    	...
	    	log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentDetails, Incident No : " + incidentNo);
	    	...
	    	String otherAcctCode = extnElm.getAttribute("ExtnOtherAcctCode");
      	...
      	result.put("IncidentCustomerId", incidentCustomerId);
				...
				log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentDetails, NULL output from getWorkOrderDetails");
			}
		}
		catch(ParserConfigurationException pce){
  ...
  throws NWCGException{
  	try {
  		log.verbose("NWCGRefurbIssueForNonSerItems::createChgIssueStatusDoc, Entered");
    	...
    	if (log.isVerboseEnabled()){
    		log.verbose("NWCGRefurbIssueForNonSerItems::createChgIssueStatusDoc, Input : " + XMLUtil.getXMLString(chgStatusDoc));
    	}
  		log.verbose("NWCGRefurbIssueForNonSerItems::createChgIssueStatusDoc, Exiting");
    	return chgStatusDoc;
  	}
  	catch (ParserConfigurationException pce){
  ...
  private String getIncidentCustomerId(YFSEnvironment env, String incidentNo){
  	String customerId = "";
		try {
			...
			if (log.isVerboseEnabled()){
				log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentCustomerId, Input XML : " + XMLUtil.getXMLString(inputDoc));
			}
			
			Document outputDoc = CommonUtilities.invokeService(env, "NWCGGetIncidentOrderService", inputDoc);
			
			if (outputDoc != null){
				if (log.isVerboseEnabled()){
					log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentCustomerId, NWCGGetIncidentOrderService : " + XMLUtil.getXMLString(outputDoc));
				}
				Element rootNode = outputDoc.getDocumentElement();
				customerId = XMLUtil.getAttribute(rootNode, "CustomerId");
			}
			else {
				log.verbose("NWCGRefurbIssueForNonSerItems::getIncidentCustomerId, NULL output from NWCGGetIncidentOrderService");
			}
		}
		catch(ParserConfigurationException pce){

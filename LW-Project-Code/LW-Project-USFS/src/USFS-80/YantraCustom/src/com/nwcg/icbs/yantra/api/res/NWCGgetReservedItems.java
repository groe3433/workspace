 ...
 * <Item IgnoreOrdering="Y" ItemID="000169" ItemIDQryType="LIKE"
 *     MaximumRecords="30" Node="CORMK" ReservationID="JSK" ReservationIDQryType="FLIKE"/>
 * 
...
public class NWCGgetReservedItems implements YIFCustomApi {
	...
	public Document getItemList(YFSEnvironment env, Document inXML) throws Exception{
		...
		if(logger.isVerboseEnabled()){
			logger.verbose("Start of NWCGgetReservedItems:getItemList API: Input XML Doc:"+XMLUtil.getXMLString(inXML));
		}
		if (env == null){
		...
		try 
		{
			...
			//System.out.println("ItemIDQryType:-> ["+ItemIDQryType+"]");
			reservationId = Item.getAttribute("ReservationID");
			//System.out.println("reservationId:-> ["+reservationId+"]");
			RsvIDQryType = rootNode.getAttribute("ReservationIDQryType");
			//System.out.println("RsvIDQryType:-> ["+RsvIDQryType+"]");
			
			//get the list of items from YFS_Inventory_Reservation by direct query.
			//-----------------
			...
			try 
			{
				...
				if (!nodeKey.equals("")) {
					if(!(reservationId.equals("")) && !(itemId.equals(""))) { // both are filled in
						if(RsvIDQryType.equals("EQ") || RsvIDQryType.equals("")) {
							if(ItemIDQryType.equals("EQ") || ItemIDQryType.equals("")) {
								rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.reservation_id = '"+reservationId+"' and a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id = '"+itemId+"'");
							...
							if(ItemIDQryType.equals("EQ") || ItemIDQryType.equals("")) {
								rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.reservation_id like '%"+reservationId+"%' and a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id = '"+itemId+"'");
							} else if (ItemIDQryType.equals("LIKE")) {
								rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.reservation_id like '%"+reservationId+"%' and a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id like '%"+itemId+"%'");
							} else if (ItemIDQryType.equals("FLIKE")) {
								rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.reservation_id like '%"+reservationId+"%' and a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id like '"+itemId+"%'");
					...
					} else if (!(reservationId.equals("")) && itemId.equals("")) { // RSVID only
						if(RsvIDQryType.equals("EQ") || RsvIDQryType.equals("")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id = '"+reservationId+"' and a.shipnode_key = '"+nodeKey+"'");
						} else if(RsvIDQryType.equals("LIKE")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id like '%"+reservationId+"%' and a.shipnode_key = '"+nodeKey+"'");
						} else if(RsvIDQryType.equals("FLIKE")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id like '"+reservationId+"%' and a.shipnode_key = '"+nodeKey+"'");
						}
					} else if (reservationId.equals("") && !(itemId.equals(""))) { // ItemID only
						if(ItemIDQryType.equals("EQ") || ItemIDQryType.equals("")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id = '"+itemId+"'");
						} else if (ItemIDQryType.equals("LIKE")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id like '%"+itemId+"%'");
						} else if (ItemIDQryType.equals("FLIKE")) {
							rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a, yfs_inventory_item b where a.shipnode_key = '"+nodeKey+"' and a.inventory_item_key = b.inventory_item_key and b.item_id like '"+itemId+"%'");
						}
					} else { // Both empty
						rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.shipnode_key = '"+nodeKey+"'");
					}
				} else {
					if(RsvIDQryType.equals("EQ")) {
						rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id = '"+reservationId+"'");	
					} else if(RsvIDQryType.equals("LIKE")) {
						rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id like '%"+reservationId+"%'");	
					} else if(RsvIDQryType.equals("FLIKE")) {
						rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where a.reservation_id like '"+reservationId+"%'");	
					} else {
						rs = stmt.executeQuery("select a.reservation_id,a.inventory_item_key, a.quantity, a.owner_key,a.shipnode_key,to_char(a.ship_date,'MM/DD/YYYY') from yfs_inventory_reservation a where 1=1");	
					}
				}
				...
				if(rs != null) {
					while (rs.next()) {
						//System.out.println("ResID:"+rs.getString(1)+ " InvItemKey:"+ rs.getString(2)+ " ResQty:"+rs.getString(3)+" Org:"+ rs.getString(4)+ " ShipNode:"+rs.getString(5)+" ShipDate:"+rs.getString(6));
			...
			Element el_ItemList=getItemListOutput_Doc.createElement("ItemList");
		...
		if(logger.isVerboseEnabled()){
			logger.verbose("Exiting the NWCGgetReservedItems:getItemList API, return document:"+XMLUtil.getXMLString(getItemListOutput_Doc));
		}
	...
	public String[] getItemName(YFSEnvironment env,String invIemKey, String OrgCode)throws Exception
	{
		...
		if(logger.isVerboseEnabled()){
			logger.verbose("Input xml:-"+ XMLUtil.getXMLString(getInventoryItemList_Input));
		}
		...
		if(logger.isVerboseEnabled()){
			logger.verbose("Output xml:-"+ XMLUtil.getXMLString(getInventoryItemList_Output));
		}

...
public class NWCGYCMValidateItemModificationUE implements
	...
	private static Logger logger = Logger
			.getLogger(NWCGYCMValidateItemModificationUE.class.getName());
	
	public void validateItemModification(YFSEnvironment env, Document inDoc)
			throws YFSUserExitException {

		if (logger.isVerboseEnabled())logger.verbose("NWCGYCMValidateItemModificationUE inDoc is: " + XMLUtil.getXMLString(inDoc));
		...
		if (strAction != null && strAction.toLowerCase().equals("delete")) {
			...
			if (nlExtn != null && nlExtn.getLength()>= 1){
				...
				strExtnPublishToRoss = eleExtn.getAttribute("ExtnPublishToRoss");
			}
			
			if (logger.isVerboseEnabled())logger.verbose("strExtnPublishToRoss is: " + strExtnPublishToRoss);
			...
			if (nlPI != null && nlPI.getLength() >= 1) {
				...
				try {
					if (logger.isVerboseEnabled())
						logger
								.verbose("calling CommonUtilities.inventoryExistsAtAnyNode with params :: + "
										+ strItemID
										+ " "
										+ strUOM
										+ " "
										+ strDefaultProductClass
										+ " "
										+ strOrgCode);
					...
					if (logger.isVerboseEnabled())logger.verbose("INXML NWCGYCMValidateItemModificationUE Got some exception while getting supply details "
										+ e);
					e.printStackTrace();
				}
			...
			if (logger.isVerboseEnabled())logger.verbose("strStatus is: " + strStatus + " and strExtnPublishToRoss is: " + strExtnPublishToRoss);

			if (strStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS) && strExtnPublishToRoss.equals(NWCGConstants.YES)){
				
				if (logger.isVerboseEnabled())logger.verbose("Got inside the loop.");
			...
			if (logger.isVerboseEnabled())
				...
				try {
					CommonUtilities.invokeService(env,NWCGConstants.NWCG_PROCESS_DELETE_ITEM_CATALOG_SERVICE, inDoc);
				}
					
			 catch (Exception e) {
				if (logger.isVerboseEnabled())logger.verbose("Caught an error on delete item.");
					e.printStackTrace();
			 	}

			}//end if
			else{
				if (logger.isVerboseEnabled())logger.verbose("Did not send delete to ROSS since item is already removed from ROSS DB.");
			}
			...
			if (nlPI != null && nlPI.getLength() >= 1) {
				Element elemPrimaryInformation = (Element) nlPI.item(0);
				strShortDescriptionNew = elemPrimaryInformation.getAttribute("ShortDescription");
			...
			NWCGItemValidator.validateItemDescription(env, strShortDescriptionNew, strItemID);
			}
			else{
				if (logger.isVerboseEnabled())logger.verbose("Short desc is blank or null");
			}
			//}
			determineUserAction(env, inDoc);
			if (logger.isVerboseEnabled())logger.verbose("action is not delete !!!!!!!!!!!!!!!!!!!!!");
		}
	}
	...
	public void determineUserAction(YFSEnvironment env, Document inputXMLDoc)
		...
		if (logger.isVerboseEnabled())logger.verbose("determineUserAction inputXMLDoc is: \n"
		...
		if (eleInputPrimaryInformation != null && eleInputExtn != null) {
			...
			if (eleInputRoot != null) {
				if (logger.isVerboseEnabled())
					logger
							.verbose("eleInputRoot is not null, creating input to getItemDetails...");
				strItemID = eleInputRoot.getAttribute("ItemID");
				strUnitOfMeasure = eleInputRoot
			...
			if (logger.isVerboseEnabled())logger.verbose("getItemDetails outDoc is: \n"
			...
			if (outDoc != null) {
				...
				eleInputPrimaryInformation.setAttribute("ShortDescriptionOld", strRetShortDescription);
				
				if (logger.isVerboseEnabled())logger.verbose("################# Action is: " + strRetAction);
				if (strRetAction.equals("")){
					if (logger.isVerboseEnabled())logger.verbose("strRetAction is blank");
				}
				
				for (int x = 0; x < 1; x++) {
					
					if (!(strInInputStatus.equals(NWCGConstants.NWCG_UNPUBLISHED_STATUS)
							&& strRetInputStatus.equals(NWCGConstants.NWCG_UNPUBLISHED_STATUS))) {
					
					if (strInInputStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS)
							&& strRetInputStatus.equals(NWCGConstants.NWCG_UNPUBLISHED_STATUS)) {

						if (logger.isVerboseEnabled())logger.verbose("strInInputStatus.equals(3000) && strRetInputStatus.equals(2000)");

						if (logger.isVerboseEnabled())logger.verbose("Status: 2000 -> 3000");

						if (strInExtnPublishToRoss.equals("Y")) {
							if (logger.isVerboseEnabled())logger.verbose("strInExtnPublishToRoss.equals(Y)");

							if (logger.isVerboseEnabled())logger.verbose("and PublishToRoss N -> Y");

							if (strAction.equals("CREATE") || strAction.equals("MODIFY")
									|| strAction.equals("")) {
								strAction = "CREATE";
							} else {
								isNoError = false;
								throw new YFSUserExitException(NWCGConstants.NWCG_MULTIPLE_STATUS_CHANGE_ERROR);
							}
				...
				if (strInInputStatus.equals(NWCGConstants.NWCG_UNPUBLISHED_STATUS) && strRetInputStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS)) {
						
					if (logger.isVerboseEnabled())logger.verbose("Status: 3000 -> 2000");
					if (logger.isVerboseEnabled())logger.verbose("strAction is: " + strAction);
						if (strAction.equals("") || strAction.equals("DELETE")){
							
							if (logger.isVerboseEnabled())logger.verbose("strAction.equals('') || strAction.equals('DELETE')");
							
							if(strRetExtnPublishToRoss.equals("Y")){
								
								if (logger.isVerboseEnabled())logger.verbose("Status: 3000 -> 2000 First delete...");
								strAction = "DELETE";
							}
						}
						else{
					...
					if (strInExtnPublishToRoss.equals("Y")
						...
						if (logger.isVerboseEnabled())logger.verbose("strInExtnPublishToRoss.equals(Y) && strRetExtnPublishToRoss.equals(N)");

						if (!(strInInputStatus.equals(NWCGConstants.NWCG_UNPUBLISHED_STATUS))) {
							if (logger.isVerboseEnabled())logger.verbose("!(strInInputStatus.equals(2000)");

							if (logger.isVerboseEnabled())logger.verbose("strRetExtnPublishToRoss: N -> Y");
							if (strAction.equals("CREATE") || strAction.equals("MODIFY")
									|| strAction.equals("")) {
								strAction = "CREATE";
							} else {
					...
					if (strInExtnPublishToRoss.equals("N")
						...
						if (logger.isVerboseEnabled())logger.verbose("strInExtnPublishToRoss.equals(N) && strRetExtnPublishToRoss.equals(Y)");

						if (logger.isVerboseEnabled())logger.verbose("strRetExtnPublishToRoss: Y -> N");

						if (strAction.equals("DELETE") || strAction.equals("")) {
							if (logger.isVerboseEnabled())logger.verbose("strRetExtnPublishToRoss: Y -> N second delete...");
							strAction = "DELETE";
						} else {
							// TODO: Throw custom error, can't make two changes
							isNoError = false;
							throw new YFSUserExitException(
					...
					if (strInExtnRossResourceItem.equals("Y")
						...
						if (logger.isVerboseEnabled())logger.verbose("strInExtnRossResourceItem.equals(Y) && strRetExtnRossResourceItem.equals(N)");

						if (logger.isVerboseEnabled())logger.verbose("strInExtnRossResourceItem: N -> Y");
						
						if ((strAction.equals("MODIFY") || strAction.equals("")) && strInExtnPublishToRoss.equals("Y") && strInInputStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS)) {
						
							strAction = "MODIFY";
							
						} 
						else if (strAction.equals("CREATE")){
							if (logger.isVerboseEnabled())logger.verbose("Action is already 'CREATE', not sending 'MODIFY'");
						}
						else if (strAction.equals("DELETE")){
							if (logger.isVerboseEnabled())logger.verbose("Action is already 'DELETE', not sending 'MODIFY'");
						}
						else {
							// TODO: Throw custom error, can't make two changes
							isNoError = false;
					...
					if (strInExtnRossResourceItem.equals("N")
						...
						if (logger.isVerboseEnabled())logger.verbose("strInExtnRossResourceItem.equals(N)	&& strRetExtnRossResourceItem.equals(Y)");

						if (logger.isVerboseEnabled())logger.verbose("strInExtnRossResourceItem: Y -> N");
						if ((strAction.equals("MODIFY") || strAction.equals("")) && (strInExtnPublishToRoss.equals("Y") && strInInputStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS))) {
					...
					if (!strRetShortDescription.equals(strInShortDescription)) {

						if (logger.isVerboseEnabled())logger.verbose("!strRetShortDescription.equals(strInShortDescription)");

						if (logger.isVerboseEnabled())logger.verbose("ShortDescription has been modified.");
					...
					if (!decInMaxOrderQuantity.equals(decRetMaxOrderQuantity)) {

						if (logger.isVerboseEnabled())logger.verbose("decInMaxOrderQuantity != decRetMaxOrderQuantity");
						if (logger.isVerboseEnabled())logger.verbose("decRetMaxOrderQuantity: "
								+ decRetMaxOrderQuantity);
						if (logger.isVerboseEnabled())logger.verbose("decInMaxOrderQuantity: "
					...
					if (!decInMinOrderQuantity.equals(decRetMinOrderQuantity)) {

						if (logger.isVerboseEnabled())logger.verbose("decInMinOrderQuantity != decRetMinOrderQuantity");
					...
					if (!strInExtnStandardPack.equals(strRetExtnStandardPack)) {
						if (logger.isVerboseEnabled())logger.verbose("strInExtnStandardPack != strRetExtnStandardPack");
						if ((strAction.equals("MODIFY") || strAction.equals("")) && (strInExtnPublishToRoss.equals("Y") && strInInputStatus.equals(NWCGConstants.NWCG_PUBLISHED_STATUS))) {
							...
							// Don't do anything. There is no need to send a message to ROSS as it is not published. If we don't have this check
							// ICBSR system will not allow to change the standard pack for an item that is not published to ROSS.
						}
						else {
							// TODO: Throw custom error, can't make two changes
							isNoError = false;
			...
			if (isNoError) {
				...
				if (logger.isVerboseEnabled())logger.verbose("User Action: " + strAction);
				if (logger.isVerboseEnabled())logger.verbose("************************************\n\n");

				if (logger.isVerboseEnabled())logger.verbose("XML being sent is...\n" + XMLUtil.getXMLString(inputXMLDoc));
				if (logger.isVerboseEnabled())logger.verbose("Action is: " + strAction);
				...
				if (strAction.equals("CREATE")) {
					try {
						...
						if (logger.isVerboseEnabled())logger.verbose("Caught an error on create item.");
						e.printStackTrace();
				...
				if (strAction.equals("MODIFY")) {
					try {
						...
						if (logger.isVerboseEnabled())logger.verbose("Caught an error on modify item.");
						e.printStackTrace();
				...
				//			throw new YFSUserExitException(NWCGConstants.NWCG_CURRENT_PENDING_SUPPLY);
				//		}
				//		else{
						if (logger.isVerboseEnabled())logger.verbose("Calling invoke delete service...");
						if (logger.isVerboseEnabled())logger.verbose("Sending DELETE the following doc:");
						if (logger.isVerboseEnabled())logger.verbose(XMLUtil.getXMLString(inputXMLDoc));
						CommonUtilities.invokeService(env,
				...
				//		}
						...
						if (logger.isVerboseEnabled())logger.verbose("There was an error while deleting the item.");
						if (logger.isVerboseEnabled())logger.verbose("Caught an error on delete item.");
						e.printStackTrace();
				...
				if (logger.isVerboseEnabled())logger.verbose(NWCGConstants.NWCG_MULTIPLE_STATUS_CHANGE_ERROR);
				throw new YFSUserExitException(NWCGConstants.NWCG_MULTIPLE_STATUS_CHANGE_ERROR);
			}
		} 
		...
		//Commented out because of conflicts with component items
			...
			if (logger.isVerboseEnabled())logger.verbose("There is no Extn Element in the input document.");
			//throw new YFSUserExitException("Component item added, or gift instructions.");

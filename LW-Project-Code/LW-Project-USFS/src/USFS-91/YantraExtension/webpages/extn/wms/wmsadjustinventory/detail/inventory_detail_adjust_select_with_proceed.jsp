...
<script language="javascript" src="<%=request.getContextPath()%>/extn/scripts/nwcg_new_adjust_inventory.js"></script>
...
function proceedClicked1(obj) {
   if(!document.all("xml:/AdjustLocationInventory/@Node").value){
		...
		//alert("KitCode ["+document.all("xml:/ItemList/Item/PrimaryInformation/@KitCode").value+"]");
		//alert("IsSerialTracked ["+document.all("xml:/ItemList/Item/InventoryParameters/@IsSerialTracked").value+"]");

		var kitCode = document.all("xml:/ItemList/Item/PrimaryInformation/@KitCode");
		var isSerialTracked = document.all("xml:/ItemList/Item/InventoryParameters/@IsSerialTracked");
		var itemUOM = document.all("xml:/ItemList/Item/@UnitOfMeasure");

		if (itemUOM.value != document.all("xml:/AdjustLocationInventory/Source/Inventory/InventoryItem/@UnitOfMeasure").value) {
			alert("The entered UOM does not exist for the item");
			doNormalWindow();
		} else {
			if (kitCode.value == "PK" && isSerialTracked.value == "Y") {
				// it's physical kit and serial tracked item
				alert("This is serial tracked KIT item. Inventory Adjustment is not allowed");
				var itemIDFld = document.getElementById("xml:/AdjustLocationInventory/Source/Inventory/InventoryItem/@ItemID");
				itemIDFld.focus();
			} else {
...
<%			lpnAPIcalled = true;
			if (isVoid(getValue("GetLPNDetails","xml:/GetLPNDetails/LPN/LPNLocation/@LocationId"))) {
				bValidLPN = false;
			}
		} %>
<%		if (bValidLPN) {
...
<%				}
			...
			if (bValidLocation) {
				if (lpnAPIcalled && locAPIcalled && 																					!equals(getValue("GetLPNDetails","xml:/GetLPNDetails/LPN/LPNLocation/@LocationId"),locationid)) { %>
					<script>
...
<tr>
	<td>
		<input type="hidden" name="xml:/ItemList/Item/PrimaryInformation/@KitCode" value=""/>
		<input type="hidden" name="xml:/ItemList/Item/InventoryParameters/@IsSerialTracked" value=""/>
		<input type="hidden" name="xml:/ItemList/Item/@UnitOfMeasure" value=""/>
	</td>
</tr>
...
<tr>
    ...
    <td nowrap="true">
		<input type="text" class="unprotectedinput" size=20 <%=getTextOptions("xml:/AdjustLocationInventory/Source/@LocationId","xml:/AdjustLocationInventory/Source/@LocationId")%> />
...
<tr>
    ...
    <td nowrap="true">
        <input type="text" class="unprotectedinput"  <%=getTextOptions("xml:/AdjustLocationInventory/Source/Inventory/InventoryItem/@ItemID","xml:/AdjustLocationInventory/Source/Inventory/InventoryItem/@ItemID")%> onblur="fetchDataWithParams(this,'getItemList',ItemDetailsForAdjInv,setItemParamsAdjInv(this));" />

...
<tr>
	...
	<%
			...
			String sShipNode = resolveValue("xml:/Order/@ShipNode");
			if( sShipNode.equals("IDGBK") ) {   
			   
		   String sOtherIssueType = getValue("Order", "xml:/Order/@OrderType");
		   
		  
		   if(sOtherIssueType.equals("CREDIT_CARD"))
		   {
			   sOtherIssueType = "Credit Card";
		   }
		   
		   if(sOtherIssueType.equals("NON_CREDIT_CARD"))
		   {
			   sOtherIssueType = "Non Credit Card";
		   }
			   
		   %>
	<td class="detaillabel" ><yfc:i18n>Order Type</yfc:i18n></td>
	<!--  <td class="protectedtext"><%=sOtherIssueType%></td> -->
		...
		<% 
			String sStatusNum =  getValue("Order", "xml:/Order/@MaxOrderStatus"); 
			double dStatusNum = 0.0;
			if ( sStatusNum != null && !sStatusNum.equals("") )
			{

				dStatusNum = Double.parseDouble(sStatusNum);
			}
		%>
		
		
		<td class="searchcriteriacell" nowrap="true">
	        <select class="combobox" <% if( dStatusNum >= 3700 ) {%> disabled <%}%> name="xml:/Order/@OrderType">
				
				<%
				
				if( sOtherIssueType.equals("") ||  sOtherIssueType == null)
				{
				...
				for(int indexOrderType = 0 ; indexOrderType < nlOrderType.getLength() ; indexOrderType++)
				{
					...
					String currentIndexDesc = elemOrderType.getAttribute("CodeShortDescription");
					
					if( sOtherIssueType.equals(currentIndexDesc) )
					{
					
				%>
					<option value='<%=elemOrderType.getAttribute("CodeValue")%>' selected="selected"> <%=elemOrderType.getAttribute("CodeShortDescription")%></option>
...
<tr>
	...
	<%
	String sOtherIssueType = getValue("Order", "xml:/Order/@OrderType");
	if(bOtherOrder && sOtherIssueType.equals("CREDIT_CARD"))
	{%>

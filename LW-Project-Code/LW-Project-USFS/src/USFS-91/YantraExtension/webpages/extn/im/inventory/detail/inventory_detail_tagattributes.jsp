						...
						for (Iterator k = currentMap.keySet().iterator(); k.hasNext();) {
							...
							if (equals(screenType, "list")) {
								...
								<% }
									...
									<td class="tablecolumn"><%=currentAttrValue%>
									  ...
									  <% } else { %>
										   ...
										   <td class=<%=inputTDClass%> nowrap="true">
												<%if(isVoid(allowModBinding)) {%>
													...
													<input type="text" class="unprotectedinput" isExtn=<%=isExtn%> <%=getTextOptions(currentBindingPrefix + "/@" +currentAttr) %> onblur="javascript:if(this.value !='' && (this.value.length != 10 || this.value.indexOf('/') != 2)){alert('Please enter a valid date in \'mm/dd/yyyy\' format');this.focus();}" />
													<%} else {%>
												...
												<%}%>
												<%if(equals(YFCDataTypeRepository.getInstance().getDataTypeForAttribute(currentAttr),"Date") || equals(currentAttr,"RevisionNo")) { %>
													<img class="lookupicon" name="search" onclick="invokeCalendar(this);return false" <%=getImageOptions(YFSUIBackendConsts.DATE_LOOKUP_ICON, "Calendar") %> />
												<%}%>
										   </td>
									  <% } %>

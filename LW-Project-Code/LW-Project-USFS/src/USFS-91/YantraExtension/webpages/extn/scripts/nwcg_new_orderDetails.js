...
function validateReqInIncidentBlock(elem) {
	...
	if (reqBlockNoStart && reqBlockNoEnd) {
		if ((parseInt(reqBlockNoStart.value) <= parseInt(reqNo)) && (parseInt(reqNo) <= parseInt(reqBlockNoEnd.value))) {
			return 0;
		} else {
			return -1;
		}
...
function checkForIncidentOrder(elem) {
	...
	//Pay.gov Changes...
	var orderType = document.getElementById("orderTypeCB");
	var cacheID = document.getElementById("xml:/Order/@ShipNode");

	// BEGIN - CR 317 - incident no, incident name, incident type and customer id should be mandatory

	// BEGIN - CR 600
    // NEW VALID CUSTOMER VARIABLE - Commented by gaurav as this attribute is not present on other order entry screen and moreover this variable is not being used anywhere in this js.
	...
	//BEGIN - CR 589 - cost center , fa , wbs check
...
function validateTrackableID(elem, xmlDoc) {
	...
	if( nodes != null && nodes.length > 0 ) {	
		...
		// BEGIN - CR 1467 - August 27, 2015
		if (strSerialStatus == 'C' && strSerialStatusDesc == 'Received as Component') {
			alert("This Trackable Kit has been returnedorderLineNL and is in a status of (Received as Components). You cannot transfer this Trackable Kit. ");
			elem.value = '';
			elem.focus();
			return false;
		}		
		// END - CR 1467 - August 27, 2015
...
function updatedIncidentDetails(elem,xmlDoc) {
	...
	populateCustomerInfo(elem,nodes,"xml:/Order/Extn/@ExtnCustomerName");
	if(nodes != null && nodes.length > 0) {	
		// We take the first NWCGIncidentOrder node (b/c there really are 2) bc a NWCGIncidentOrder element exists underneath the NWCGRossAccountCodesList element returned from Get Incident for Issue Creation SDF service
		var incidentOrder = nodes(0);
...
function checkHoldForScheduleAndRelease(viewId) {
	// BEGIN - CR 18 - Shipping Contact Name and Phone are Mandatory fields
...
// BEGIN CR 442 - check available rfi
...
function populateItemForCacheRequestDetails(elem,xmlDoc) {
	var nodes=xmlDoc.getElementsByTagName("Item");
	var strPC = '';
	var strQty = '';
	var strUOM = '';
	...
	var label = currentRow.getElementsByTagName("label");
	if(label != null && label != 'undefined' && label.length > 0 ) {
		// BEGIN - CR 1711 - JUNE 29, 2015
		label(0).style.fontSize="20px";
		// END - CR 1711 - JUNE 29, 2015
		label(0).innerText= strDesc;
	}
	var InputList = currentRow.getElementsByTagName("Input");
	for (i = 0 ; i < InputList.length ; i++) {
...
function populateItemForCacheTransferDetails(elem,xmlDoc) {
	...
	var label = currentRow.getElementsByTagName("label");
	if(label != null && label != 'undefined' && label.length > 0 ) {
		// BEGIN - CR 1771 - JUNE 29, 2015
		label(0).style.fontSize="11px";
		// END - CR 1771 - JUNE 29, 2015
		label(0).innerText= strDesc;
	}
	var InputList = currentRow.getElementsByTagName("Input");
	for (i = 0 ; i < InputList.length ; i++) {
	...
	if (extnNavInfo != null && extnNavInfo != 'undefined' && oldNavInfo !=null && oldNavInfo != 'undefined') {
		if (extnNavInfo.value == 'WILL_PICK_UP') {
			...
			if (extnShippingInstrCity != null && extnShippingInstrCity != 'undefined' && extnShippingInstrCity.value != '' &&
				...
				if (((addrLine1) && (addrLine1 == 'undefined' || addrLine1.value == '')) || ((state) && (state == 'undefined' || state.value == ''))) {
				alert('Please enter SHIP TO address details');
				return false;
			}					
		}

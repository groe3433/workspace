...
// this function will populate the order lines - a call back function from AJAX framework steps 
// 1. clear the current table
// 2. populate the rows
// 3. render the data in table	
...
// this function checks if the page number entered is more then the total number of pages if yes then we wont submit the request to fetch the lines from the server
...
function setOrderLineParam(elem,orderHeaderKey,strMax) {
	...
	if(elem.value == 'Next') {
		var objNext = document.getElementById("Previous");
		objNext.disabled = false ;
		// increase the page number - just to pass on the value the textbox will display the value only when the call back function is invoked

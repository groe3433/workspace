...
<table class="table" width="100%" >
  ...
  <tbody> 
	  ...
	  <yfc:loopXML  binding="xml:/Inbox/InboxReferencesList/@InboxReferences" id="InboxReferences"> 
		...
		%>
		<%
		if (referenceName.equalsIgnoreCase("Distribution ID") || referenceName.equalsIgnoreCase("DistributionID")){
		%>
			<input type="hidden" name="xml:/Inbox/InboxReferencesList/InboxReferences/@ReferenceType" value="<%=referenceType%>"/>
			<input type="hidden" name="xml:/Inbox/InboxReferencesList/InboxReferences/@Name" value="<%=referenceName%>"/>
			<input type="hidden" name="xml:/Inbox/InboxReferencesList/InboxReferences/@Value" value="<%=referenceValue%>"/>
		<%
		}
		%>
	  ...
	  <% if(!(linkNo !=null && keyName != null && keyName.equals(referenceName))){ %>
			<%if ( "URL".equalsIgnoreCase(referenceType) ) {%>
				...
				<td class="tablecolumn"><a href='<%=referenceValue%>' >click</a></td>
			<%} else if ("TraceXML".equalsIgnoreCase(referenceType)) {
				...
				<tr class='<%=className%>'>
					<td>
						<img onclick="expandCollapseDetails('optionSet_<%=InboxReferencesCounter%>','<%=getI18N("Click_To_See_Alert_Detail_Info")%>','<%=getI18N("Click_To_Hide_Alert_Detail_Info")%>','<%=YFSUIBackendConsts.FOLDER_COLLAPSE%>','<%=YFSUIBackendConsts.FOLDER_EXPAND%>')"  style="cursor:hand" <%=getImageOptions(YFSUIBackendConsts.FOLDER,"Click_To_See_Alert_Detail_Info")%> />
					</td>
					<td class="tablecolumn" ><%=getLocalizedValue(referenceName,getReferenceValueHeader(referenceValue))%></td>

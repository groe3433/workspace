...
public class NWCGCancelTrackableItem implements YIFCustomApi {
	...
	private Document updateSerialStatus(YFSEnvironment env, Document inDoc)
		...
		inDocrootElement.setAttribute("TrackableItemKey",
				rootElement.getAttribute("TrackableItemKey"));
		inDocrootElement.setAttribute("SerialStatusDesc",
				getSerialStatusDesciption(rootElement
						.getAttribute("ReasonForCancellation")));
		inDocrootElement.setAttribute("SerialStatus",
				rootElement.getAttribute("ReasonForCancellation"));
		inDocForService.appendChild(inDocrootElement);

		Document outDoc = CommonUtilities.invokeService(env,
				"NWCGChangeTrackableInventoryRecordService", inDocForService);
		return outDoc;
	}

	private String getSerialStatusDesciption(String serialStatus) {
		String serialStatusDesc = null;
		if (serialStatus.equals("L"))
			serialStatusDesc = "Lost/Stolen";
		else if (serialStatus.equals("X"))
			serialStatusDesc = "Destroyed";
		else if (serialStatus.equals("R"))
			serialStatusDesc = "Transferred";
		else if (serialStatus.equals("E"))
			serialStatusDesc = "Issued";
		else if (serialStatus.equals("O"))
			serialStatusDesc = "Other";
